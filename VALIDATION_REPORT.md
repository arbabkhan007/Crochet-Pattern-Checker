...
**Deliverables:** 20 PDFs + 11 HTML pages (10 patterns + catalog) in `deliverables/`,
plus `Novality_Crochet_Patterns_Bundle.zip` (PDF + HTML + validated sources + report).
...
**HTML editions** (`deliverables/html/`, generated by `tools/generate_html.py` from the same
spec data as the PDFs — the two formats can never drift): one self-contained, mobile-friendly,
print-CSS page per pattern with the cover inlined, plus `index.html` catalog.
`Novality_Crochet_Patterns_Bundle.zip` (23.8 MB, 43 files) packages all of it with the
validated sources and this report — zip integrity tested OK.
