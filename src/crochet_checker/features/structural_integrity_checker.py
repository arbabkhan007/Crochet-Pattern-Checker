...
class StructuralIntegrityChecker:
    def __init__(self):
        self.strength_factors = {
            "sc": 0.9, "dc": 0.7, "hdc": 0.8, "tr": 0.6,
            "inc": 0.8, "dec": 0.9, "sl_st": 0.5,
        }
    ...
    def check_integrity(self, pattern_text: str) -> dict:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if avg_strength < 0.6:
                weak_points.append({"row": i, "strength": avg_strength, "issue": "Low structural strength"})
        ...
        return {
            ...
            "overall_strength": "high" if avg_integrity > 0.8 else "medium" if avg_integrity > 0.6 else "low",
    ...
    def _get_recommendations(self, score: float, weak_points: list) -> list:
        ...
        if weak_points:
            recommendations.append(f"{len(weak_points)} weak rows detected - reinforce these areas")
