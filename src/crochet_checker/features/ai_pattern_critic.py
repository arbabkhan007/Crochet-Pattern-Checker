...
class AIPatternCritic:
    ...
    def review_pattern(self, pattern_text: str) -> dict:
        ...
        clarity_score = 10
        if any(
            "do stuff" in line.lower() or "make it" in line.lower() for line in lines
        ):
        ...
        if not any(
            "materials" in line.lower() or "supplies" in line.lower() for line in lines
        ):
        ...
        if abbreviations:
            suggestions.append(
                f"Define abbreviations: {', '.join(list(abbreviations)[:3])}"
            )
        ...
        if len(lines) > 10:
            if not any(
                line.strip().startswith(("Row", "Round", "Materials", "Instructions"))
                for line in lines
            ):
                suggestions.append(
                    "Add clear section headers (Materials, Instructions, etc.)"
                )
                formatting_score -= 2
