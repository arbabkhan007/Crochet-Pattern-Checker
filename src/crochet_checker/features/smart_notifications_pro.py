...
class SmartNotificationsPro:
    ...
    def add_wip_reminder(
        self, project_name: str, last_worked: str, reminder_days: int = 7
    ):
        ...
        notification = {
            ...
            "reminder_date": (
                datetime.strptime(last_worked, "%Y-%m-%d")
    ...
    def add_deadline(self, project_name: str, deadline: str, advance_notice: int = 3):
        ...
        notification = {
            ...
            "deadline": deadline,
            "notice_date": (
                datetime.strptime(deadline, "%Y-%m-%d") - timedelta(days=advance_notice)
            ).strftime("%Y-%m-%d"),
    ...
    def get_upcoming_notifications(self, days_ahead: int = 7) -> list[dict]:
        ...
        for notif in self.data["notifications"]:
            ...
            date_str = (
                notif.get("reminder_date")
                or notif.get("notice_date")
                or notif.get("date")
            )
            if not date_str:
                continue
            ...
            if today <= notif_date <= future:
                upcoming.append({**notif, "days_away": (notif_date - today).days})
        ...
        if upcoming:
            ...
            for notif in upcoming:
                if notif["days_away"] == 0:
                    ...
                    briefing += (
                        f"  📌 {notif['message']} (in {notif['days_away']} day(s))\n"
                    )
        else:
...
if __name__ == "__main__":
    ...
    notifier.add_wip_reminder(
        "Granny Square Blanket", (today - timedelta(days=5)).strftime("%Y-%m-%d"), 7
    )
    notifier.add_deadline(
        "Birthday Gift Scarf", (today + timedelta(days=10)).strftime("%Y-%m-%d"), 3
    )
    notifier.add_custom_reminder(
        "Yarn sale at Joann's!",
