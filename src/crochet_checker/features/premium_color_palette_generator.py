...
class PremiumColorPaletteGenerator:
    ...
    def generate_palette(
        self, base_color: str, palette_type: str = "analogous"
    ) -> list:
        ...
        if palette_type == "analogous":
            for i in range(5):
                ...
                palette.append(
                    self._rgb_to_hex(int(r * 255), int(g * 255), int(b * 255))
                )
    ...
    def get_premium_palette(self, palette_name: str) -> list:
        ...
        return self.premium_palettes.get(
            palette_name, self.premium_palettes["luxury_neutrals"]
        )
