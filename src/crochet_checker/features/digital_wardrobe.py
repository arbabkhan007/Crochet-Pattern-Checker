...
class WardrobeItem:
    ...
    @property
    def cost_per_wear(self) -> float | None:
...
class DigitalWardrobe:
    ...
    def get_wardrobe_stats(self) -> dict:
        ...
        best_value = sorted(
            worn_items, key=lambda i: i.cost / i.times_worn if i.times_worn > 0 else 999
        )[:5]
        ...
        return {
            ...
            "most_worn": [
                {"name": i.name, "wears": i.times_worn, "cpw": i.cost_per_wear}
                for i in most_worn
            ],
            "best_value": [
                {"name": i.name, "cpw": i.cost_per_wear, "cost": i.cost}
                for i in best_value
            ],
            "avg_rating": round(avg_rating, 1) if rated_items else 0,
            "items_by_season": {
                s: len([i for i in items if s in i.tags]) for s in self.SEASONS
            },
        }
    ...
    def get_yarn_usage(self) -> dict:
        ...
        return {
            ...
            "details": {
                k: {"items": v["items"], "categories": list(v["categories"])}
                for k, v in yarn_stats.items()
            },
            "most_used_yarn": max(yarn_stats.items(), key=lambda x: x[1]["items"])[0]
            if yarn_stats
            else "None",
        }
