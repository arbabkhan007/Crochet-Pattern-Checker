...
class ErgonomicAssistant:
    ...
    STRETCHES = [
        ...
        StretchExercise(
            "Neck Roll", "neck", 30, "Slowly roll head in circles, 5 each direction"
        ),
        StretchExercise(
            "Shoulder Shrug",
            "shoulders",
        ...
        StretchExercise(
            "Wrist Circles", "wrists", 15, "Rotate wrists in circles, 10 each direction"
        ),
        StretchExercise(
            "Finger Taps",
            "hands",
    ...
    def check_break_needed(self) -> dict:
        ...
        schedule = self.BREAK_SCHEDULE.get(
            self.data["intensity"], self.BREAK_SCHEDULE["moderate"]
        )
        time_since_break = elapsed - (
            self.data["breaks_taken"]
            * (schedule["work_minutes"] + schedule["break_minutes"])
        )
        ...
        return {
            ...
            "next_break_in": round(schedule["work_minutes"] - time_since_break, 1)
            if not needed
            else 0,
            "schedule": schedule,
            "message": "Time for a break! 🧘"
            if needed
            else f"Next break in {round(schedule['work_minutes'] - time_since_break, 1)} min",
        }
    ...
    def take_break(self) -> dict:
        ...
        selected = random.sample(
            self.STRETCHES, min(num_stretches, len(self.STRETCHES))
        )
    ...
    def log_pain(self, area: str, severity: int, notes: str = "") -> dict:
        ...
        return {
            ...
            "severity": severity,
            "advice": advice,
            "pattern": f"This is the {len(recent)}th time you've logged {area} pain"
            if len(recent) > 2
            else "",
    ...
    def _get_pain_advice(self, area: str, severity: int) -> list[str]:
        ...
        area_advice = {
            "thumb": [
                "Try a larger hook grip",
                "Use your other hand more",
                "Thumb splint at night",
            ],
            "wrist": [
                "Check your wrist angle",
                "Use ergonomic hooks",
                "Wrist exercises 3x daily",
            ],
            "forearm": [
                "Massage forearm muscles",
                "Check tension - relax your grip",
                "Take more breaks",
            ],
            "shoulder": [
                "Check your seating position",
                "Do shoulder rolls every 15 min",
                "Support your arms",
            ],
            "neck": [
                "Raise your work to eye level",
                "Take posture breaks",
    ...
    def get_health_report(self) -> dict:
        ...
        for area, data in by_area.items():
            data["avg_severity"] = round(
                sum(e["severity"] for e in data["entries"]) / data["count"], 1
            )

        worst_area = (
            max(by_area.items(), key=lambda x: x[1]["count"]) if by_area else (None, {})
        )
...
if __name__ == "__main__":
    ...
    for s in break_info["stretches"]:
        print(
            f"  • {s['name']} ({s['target']}, {s['duration']}): {s['description'][:60]}..."
        )
    ...
    if report["worst_area"]:
        print(
            f"  Worst area: {report['worst_area']} ({report['worst_area_count']} times)"
        )
