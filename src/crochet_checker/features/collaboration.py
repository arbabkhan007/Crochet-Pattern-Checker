...
class CollaborationManager:
    ...
    def get_shared_pattern(self, share_id: str) -> SharedPattern | None:
    ...
    def add_comment(
        self, share_id: str, author: str, text: str, round_number: int | None = None
    ) -> str | None:
        """
        ...
        comment_id = hashlib.md5(
            f"{author}{text}{datetime.now().isoformat()}".encode()
        ).hexdigest()[:8]

        comment = Comment(
            id=comment_id, author=author, text=text, round_number=round_number
        )
    ...
    def get_comments(
        self, share_id: str, unresolved_only: bool = False
    ) -> list[Comment]:
    ...
    def add_review(
        ...
        review_id = hashlib.md5(
            f"{reviewer}{rating}{datetime.now().isoformat()}".encode()
        ).hexdigest()[:8]
    ...
    def get_average_rating(self, share_id: str) -> float | None:
    ...
    def get_stats(self) -> dict:
        ...
        total_collaborators = sum(
            len(sp.collaborators) for sp in self.shared_patterns.values()
        )
        ...
        ratings = [
            r["rating"] for sp in self.shared_patterns.values() for r in sp.reviews
        ]
...
class PatternCommunity:
    ...
    def browse_patterns(
        ...
        if sort_by == "rating":
            patterns.sort(
                key=lambda p: self.collab.get_average_rating(p.id) or 0, reverse=True
            )
    ...
    def get_trending_patterns(self, days: int = 7) -> list[dict]:
        ...
        for p in patterns:
            ...
            activity_score = (
                len(p.comments) * 2 + len(p.reviews) * 3 + len(p.collaborators) * 1
            )
    ...
    def get_top_contributors(self, limit: int = 10) -> list[dict]:
        ...
        contributors = sorted(
            user_stats.values(), key=lambda x: x["patterns_shared"], reverse=True
        )
...
╚══════════════════════════════════════════════════════════╝
    ...
    # Add comments
    c1 = collab.add_comment(
        share_id,
        "yarn_lover_123",
        "Love this pattern! The bear turns out adorable!",
        round_number=5,
    )
    c2 = collab.add_comment(
        share_id,
        "stitch_master",
        "Round 12 was a bit tricky, but got it after practice",
    )
    c3 = collab.add_comment(
        share_id, "crafty_kate", "Thanks for the feedback! Glad you like it!"
    )

    print("✅ Added 3 comments")

    # Resolve a comment
    collab.resolve_comment(share_id, c1)
    print("✅ Resolved 1 comment")

    print("\n⭐ ADDING REVIEWS")
    print("═══════════════════════════════════════════════════════════")

    # Add reviews
    r1 = collab.add_review(
        share_id,
        "yarn_lover_123",
        5,
        "Amazing pattern!",
    ...
    for p in public:
        ...
        print(
            f"   • {p['shared_by']}'s pattern - Rating: {rating}/5, Comments: {p['comment_count']}"
        )
    ...
    print("✅ Exported collaboration data:")
