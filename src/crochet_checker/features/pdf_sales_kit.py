...
from _pdf_utils import setup_unicode_font
...
class PDFSalesKit:
    ...
    def create_terms_page(self) -> None:
        ...
        terms = [
            ...
            (
                "NO SHARING",
                "Please do not share, redistribute, or resell this pattern.",
            ),
            (
                "FINISHED ITEMS",
                "You may sell finished items made from this pattern in small quantities (up to 50). Please credit the designer.",
            ),
            (
                "NO MASS PRODUCTION",
                "Mass production of finished items is not permitted.",
            ),
            (
                "PATTERN RIGHTS",
                "The pattern design, photos, and instructions are copyright protected.",
            ),
            (
                "REFUNDS",
                "Due to the digital nature of this product, refunds are not available after download.",
            ),
    ...
    def create_pricing_sheet(self, patterns: list[dict]) -> None:
        ...
        for i, pattern in enumerate(patterns):
            ...
            self.pdf.cell(
                30, 7, pattern.get("difficulty", ""), border=1, align="C", fill=True
            )
            self.pdf.cell(
                20, 7, str(pattern.get("pages", "")), border=1, align="C", fill=True
            )
            self.pdf.cell(
                20, 7, str(pattern.get("yards", "")), border=1, align="C", fill=True
            )
            self.pdf.cell(
                30, 7, f"${pattern.get('price', '')}", border=1, align="C", fill=True
            )
            self.pdf.ln()
    ...
    def generate_sales_kit(self, patterns: list[dict], output_path: str) -> str:
...
if __name__ == "__main__":
    ...
    print("   Includes: Listing previews, pricing, social media, terms")
