...
class SmartYarnSubstitution:
        ...
        for yarn in self.yarn_db:
            ...
            if yarn.weight_category == original_weight:
                ...
                if abs(w1 - w2) == 1:
                    score += 15
                    reasons.append(
                        f"Close weight ({yarn.weight_category} vs {original_weight})"
                    )
                    warnings.append("Different weight - adjust hook size")
                else:
                    score += 0
                    warnings.append("Very different weight - not recommended")
            ...
            if original_fiber and original_fiber.lower() in yarn.fiber.lower():
                ...
                "acrylic" in original_fiber.lower() and "acrylic" in yarn.fiber.lower()
            ):
                score += 20
                ...
                if original_fiber and yarn.fiber != original_fiber:
                    warnings.append(
                        f"Different fiber ({yarn.fiber} vs {original_fiber})"
                    )
    ...
    def _find_yarn(self, name: str) -> YarnProfile | None:
        """Find a yarn by name"""
    ...
    def suggest_for_project(
        self, project_type: str, difficulty: str = "Beginner", budget: str = "$"
    ) -> list[dict]:
...
if __name__ == "__main__":
    ...
    results = sub.find_substitutes(
        yarn_name="Red Heart Super Saver", project_type="blankets"
    )
