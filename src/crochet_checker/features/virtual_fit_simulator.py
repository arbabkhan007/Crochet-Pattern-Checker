...
class VirtualFitSimulator:
    ...
    def simulate_fit(self, pattern_dimensions: dict, body_size: str = "medium") -> dict:
        ...
        waist_fit = self._calculate_fit(
            pattern_dimensions.get("waist", 0), body["waist"]
        )
        ...
        return {
            ...
            "recommendations": self._get_fit_recommendations(
                overall_fit, bust_fit, waist_fit, hips_fit
            ),
        }

    def _calculate_fit(
        self, garment_measurement: float, body_measurement: float
    ) -> dict:
    ...
    def _get_fit_recommendations(
        self, overall: float, bust: dict, waist: dict, hips: dict
    ) -> list:
...
if __name__ == "__main__":
    ...
    print("\nDetailed Fit:")
    print(
        f"  Bust: {result['bust_fit']['status']} ({result['bust_fit']['ease_cm']:+.1f} cm ease)"
    )
    print(
        f"  Waist: {result['waist_fit']['status']} ({result['waist_fit']['ease_cm']:+.1f} cm ease)"
    )
    print(
        f"  Hips: {result['hips_fit']['status']} ({result['hips_fit']['ease_cm']:+.1f} cm ease)"
    )
