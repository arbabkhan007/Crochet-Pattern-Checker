...
class CustomSizingCalculator:
    ...
    def calculate_gauge(
        ...
        swatch_stitches: int,
        swatch_inches: float,
        swatch_rows: int = None,
        swatch_rows_inches: float = None,
    ) -> dict:
    ...
    def match_standard_size(
        self, bust: float = None, waist: float = None, hip: float = None
    ) -> dict:
    ...
    def calculate_custom_sizing(
        self,
        body_measurement: float,
        gauge_sts_per_inch: float,
        ease: float = 2,
        garment_part: str = "bust",
    ) -> dict:
    ...
    def full_sizing_plan(
        self,
        measurements: dict,
        gauge: dict,
        garment_type: str = "sweater",
        fit: str = "semi_fitted",
    ) -> dict:
        ...
        for area in garment["areas"]:
            ...
            plan[area] = {
                ...
                "stitches": sts_needed,
                "rows": math.ceil(total_inches * gauge.get("rows_per_inch", 5))
                if gauge.get("rows_per_inch")
                else None,
            }
...
if __name__ == "__main__":
    ...
    # Gauge
    print("\n📏 Calculating Gauge:")
    gauge = calc.calculate_gauge(
        swatch_stitches=16, swatch_inches=4, swatch_rows=12, swatch_rows_inches=4
    )
    ...
    print(
        f'  Recommended: {match["recommended_size"]} (avg diff: {match["avg_difference"]}")'
    )

    # Custom sizing
    print('\n🧮 Custom Sizing (Bust = 37", ease = 4"):')
    sizing = calc.calculate_custom_sizing(
        37, gauge["sts_per_inch"], ease=4, garment_part="bust"
    )
    ...
    for area, data in plan["plan"].items():
        print(
            f'  {area}: Body {data["body"]}" + Ease {data["ease"]}" = {data["finished"]}" → {data["stitches"]} sts'
        )
