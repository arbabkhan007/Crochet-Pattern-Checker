...
class VoiceAssistant:
    ...
    COMMANDS = [
        VoiceCommand(
            "next", "next_round", ["n", "next round", "continue"], "Go to next round"
        ),
        VoiceCommand(
            "previous",
            "prev_round",
        ...
        VoiceCommand(
            "speed up", "faster", ["faster", "quicker"], "Increase reading speed"
        ),
        VoiceCommand(
            "slow down", "slower", ["slower", "slow down"], "Decrease reading speed"
        ),
        VoiceCommand(
            "round", "jump_round", ["go to round", "jump to"], "Jump to specific round"
        ),
        VoiceCommand(
            "status",
            "status",
            ["where am i", "progress", "how far"],
            "Show current position",
        ),
        VoiceCommand(
            "count", "count_stitches", ["how many", "stitch count"], "Show stitch count"
        ),
        VoiceCommand(
            "help", "help", ["commands", "what can you do"], "Show available commands"
        ),
        VoiceCommand(
            "stop", "stop_session", ["exit", "quit", "done"], "End reading session"
        ),
        VoiceCommand(
            "note",
            "add_note",
            ["remember", "make a note"],
            "Add a note for current round",
        ),
        VoiceCommand(
            "bookmark", "bookmark", ["save place", "mark"], "Bookmark current position"
        ),
    ]
    ...
    def get_stitch_count(self) -> str:
        ...
        if 0 <= idx < len(self.rounds):
            ...
            count = rnd.get("stitch_count", rnd.get("count", 0))
            rn = rnd.get("round_number", rnd.get("round", self.session.current_round))
            return self._format_announcement(f"Round {rn} has {count} stitches.")
    ...
    def add_note(self, note: str) -> str:
        ...
        self.session.notes.append(f"Round {self.session.current_round}: {note}")
        return self._format_announcement(
            f"Note saved for Round {self.session.current_round}."
        )
    ...
    def process_command(self, command: str) -> str:
        ...
        for vc in self.COMMANDS:
            if cmd_lower == vc.command.lower() or cmd_lower in [
                a.lower() for a in vc.aliases
            ]:
                action = vc.action
...
if __name__ == "__main__":
    ...
    sample = {
        ...
        "rounds": [
            ...
            {
                "round": 3,
                "instruction": "single crochet, increase repeat around",
                "count": 18,
            },
            {
                "round": 4,
                "instruction": "2 single crochet, increase repeat around",
                "count": 24,
            },
            {
                "round": 5,
                "instruction": "single crochet in each stitch around",
                "count": 24,
            },
            {
                "round": 6,
                "instruction": "single crochet, decrease repeat around",
                "count": 18,
            },
            {
                "round": 7,
                "instruction": "2 single crochet, decrease repeat around",
                "count": 12,
            },
