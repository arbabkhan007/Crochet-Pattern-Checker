...
class AIErrorDetector:
            ...
            if re.match(r"^row\s+\d+[:\.]\s+(sc|dc|hdc|tr)", line_lower, re.IGNORECASE):
                ...
                if i > 0 and "turn" not in lines[i - 1].lower():
                    errors.append(
                        {
                            "type": "missing_turn",
                            "message": "Row starts with stitch but previous row doesn't say 'turn'",
                            "line": f"Line {i + 1}",
                            "auto_fix": True,
    ...
    def auto_fix(self, pattern_text: str) -> str:
        ...
        fixed = re.sub(
            r"(row\s+\d+[:\.]\s+)(dc\b)", r"\g<1>ch 3, \2", fixed, flags=re.IGNORECASE
        )

        fixed = re.sub(
            r"(row\s+\d+[:\.]\s+)(tr\b)", r"\g<1>ch 4, \2", fixed, flags=re.IGNORECASE
        )
