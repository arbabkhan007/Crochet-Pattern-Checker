...
class PatternMarketplace:
    ...
    def search(
        ...
        for listing in self.listings.values():
            ...
            if (
                query
                and query_lower
                not in f"{listing.title} {listing.description} {' '.join(listing.tags)}".lower()
            ):
                continue
    ...
    def add_review(
        self, listing_id: str, reviewer: str, rating: int, text: str = ""
    ) -> bool:
    ...
    def get_revenue_report(self, designer: str) -> dict:
        ...
        return {
            ...
            "avg_rating": round(
                sum(l.average_rating for l in patterns) / max(1, len(patterns)), 1
            ),
            "top_pattern": max(patterns, key=lambda l: l.downloads).title
            if patterns
            else "N/A",
        }
...
if __name__ == "__main__":
    ...
    for r in results:
        print(
            f"  • {r.title} by {r.designer} - {'FREE' if r.is_free else '$' + str(r.price)} ({r.downloads} downloads)"
        )
