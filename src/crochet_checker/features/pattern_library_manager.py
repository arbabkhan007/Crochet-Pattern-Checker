...
class EnhancedPatternLibrary:
    ...
    def search_patterns(self, query: str) -> list[Pattern]:
        ...
        for pattern in self.patterns.values():
            if (
                query_lower in pattern.title.lower()
                or query_lower in pattern.content.lower()
    ...
    def get_statistics(self) -> dict:
        ...
        for pattern in patterns:
            ...
            difficulties[pattern.difficulty] = (
                difficulties.get(pattern.difficulty, 0) + 1
            )
...
def format_pattern_list(patterns: list[Pattern]) -> str:
    ...
    output = ["📚 Pattern Library", "=" * 40, f"Found {len(patterns)} pattern(s)\n"]
