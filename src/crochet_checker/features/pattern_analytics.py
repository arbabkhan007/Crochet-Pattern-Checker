...
class PatternAnalyticsEngine:
    def analyze(self, pattern_text: str, title: str = "Pattern") -> PatternAnalytics:
        ...
        total_rounds = (
            len(set(rounds))
            if rounds
            else len([l for l in lines if "round" in l.lower()])
        )
        ...
        difficulty_factors = self._identify_difficulty_factors(
            pattern_text, stitch_counts
        )
    ...
    def _calculate_complexity(
        self, stitch_counts: dict[str, int], total_rounds: int
    ) -> float:
        score = 0.0
    ...
    def _identify_difficulty_factors(
        self, pattern_text: str, stitch_counts: dict[str, int]
    ) -> list[str]:
        factors = []
...
def format_analytics(analytics: PatternAnalytics) -> str:
    ...
    for stitch, count in sorted(
        analytics.stitch_distribution.items(), key=lambda x: x[1], reverse=True
    ):
        percentage = (
            (count / analytics.total_stitches * 100)
            if analytics.total_stitches > 0
            else 0
        )
