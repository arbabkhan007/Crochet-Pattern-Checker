...
class ComplexGaugeCalculator:
    ...
    def calculate_complex_gauge(self, swatch_measurements: dict, pattern_type: str) -> dict:
        """Calculate gauge for complex patterns"""
        pattern_info = self.pattern_types.get(pattern_type, self.pattern_types['texture'])
        ...
        adjusted_gauge = {
            'stitches_per_4in': raw_gauge['stitches_per_4in'] * pattern_info['stretch_factor'],
    ...
    def calculate_project_dimensions(self, gauge: dict, stitch_count: int, row_count: int) -> dict:
...
if __name__ == "__main__":
    ...
    print(f"Raw Gauge: {result['raw_gauge']['stitches_per_4in']:.1f} sts x {result['raw_gauge']['rows_per_4in']:.1f} rows")
    print(f"Adjusted Gauge: {result['adjusted_gauge']['stitches_per_4in']:.1f} sts x {result['adjusted_gauge']['rows_per_4in']:.1f} rows")
    ...
    dimensions = calculator.calculate_project_dimensions(result['adjusted_gauge'], 100, 120)
    print(f"\nProject Dimensions (100 sts x 120 rows):")
    print(f"  Width: {dimensions['width_inches']:.1f} inches ({dimensions['width_cm']:.1f} cm)")
    print(f"  Height: {dimensions['height_inches']:.1f} inches ({dimensions['height_cm']:.1f} cm)")
