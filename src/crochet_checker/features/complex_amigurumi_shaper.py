...
class ComplexAmigurumiShaper:
    ...
    def design_complex_shape(self, shape_type: str, dimensions: dict) -> dict:
        ...
        if shape_type == 'sphere':
            ...
            rounds = self._generate_oval_rounds(dimensions.get('length', 12), dimensions.get('width', 8))
    ...
    def _generate_sphere_rounds(self, diameter: int) -> list:
        ...
        for i in range(1, 7):
            ...
            rounds.append({
                'round': len(rounds) + 1,
                'instruction': f"inc in each stitch around",
                'count': count
            })
        ...
        for i in range(4):
            rounds.append({
                'round': len(rounds) + 1,
                'instruction': f"sc in each stitch around",
                'count': max_count
            })
        ...
        for i in range(6, 0, -1):
            ...
            rounds.append({
                'round': len(rounds) + 1,
                'instruction': f"dec evenly around",
                'count': count
            })
    ...
    def _generate_oval_rounds(self, length: int, width: int) -> list:
        ...
        for i in range(2, 6):
            rounds.append({
                'round': i,
                'instruction': f"inc at each end, sc around",
    ...
    def _generate_complex_rounds(self, dimensions: dict) -> list:
        ...
        rounds.append({
            'round': 1,
            'instruction': "magic ring, 6 sc",
            'count': 6
        })
        rounds.append({
            'round': 2,
            'instruction': "inc in each around",
            'count': 12
        })
        rounds.append({
            'round': 3,
            'instruction': "*sc 2, inc* around",
            'count': 18
        })
        rounds.append({
            'round': 4,
    ...
    def _get_assembly_notes(self, shape_type: str) -> list:
        notes = {
            'sphere': ['Stuff firmly before closing', 'Use stitch markers for symmetry'],
            'oval': ['Stuff ends first', 'Shape while stuffing'],
            'complex': ['Stuff in sections', 'Add features before assembly', 'Use pins to hold parts']
        }
