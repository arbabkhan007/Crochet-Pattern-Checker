...
class CrochetTimeCapsule:
    ...
    def create_capsule(
        ...
        if open_date is None:
            open_date = (datetime.now() + timedelta(days=open_days)).strftime(
                "%Y-%m-%d"
            )
    ...
    def open_capsule(self, capsule_id: str) -> dict:
        """Open a time capsule"""
        capsule = next(
            (c for c in self.data["capsules"] if c["id"] == capsule_id), None
        )
        if not capsule:
        ...
        if capsule["open_date"] > datetime.now().strftime("%Y-%m-%d"):
            days_left = (
                datetime.strptime(capsule["open_date"], "%Y-%m-%d") - datetime.now()
            ).days
        ...
        return {
            ...
            "days_sealed": (
                datetime.now() - datetime.strptime(capsule["created"], "%Y-%m-%d")
            ).days,
        }
    ...
    def get_journey_stats(self) -> dict:
        ...
        checkable = [
            ...
            if p["check_date"] <= today and p["came_true"] is None
        ]
        ...
        return {
            ...
            "accuracy": f"{round(correct / max(1, len(checked)) * 100)}%"
            if checked
            else "N/A",
        }
    ...
    def get_year_in_review(self) -> dict:
        ...
        year_predictions = [
            p for p in self.data["predictions"] if p["date"] >= year_ago
        ]
...
if __name__ == "__main__":
    ...
    # Take snapshots
    s1 = capsule.create_snapshot(
        "Granny Square Blanket",
        "Working on round 12, using 6 colors",
        current_skill="intermediate",
    )
    s2 = capsule.create_snapshot(
        "Amigurumi Bunny", "Finally got the ears right!", current_skill="intermediate"
    )
    print("✅ Created 2 snapshots")

    # Make predictions
    p1 = capsule.make_prediction(
        "I'll learn to read patterns without counting every stitch", "skill"
    )
