...
class VirtualBlockingSimulator:
    ...
    def simulate_blocking(self, current_size: dict, target_size: dict, yarn_type: str = "wool") -> dict:
        """Simulate blocking process"""
        yarn_props = self.fabric_properties.get(yarn_type, self.fabric_properties["wool"])
        ...
        can_achieve = (abs(width_change) <= max_stretch and 
                      abs(height_change) <= max_stretch)
        ...
        return {
            ...
            "recommendation": "Blocking will work" if can_achieve else "Target size not achievable"
        }
