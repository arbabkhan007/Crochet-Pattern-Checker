...
class CrochetEventPlanner:
    ...
    def add_rsvp(
        self, event_id: str, name: str, email: str = "", attending: bool = True
    ) -> dict:
        """Add an RSVP"""
    ...
    def calculate_budget(
        ...
        if price > 0 and supply_cost > 0:
            break_even = (
                round(venue_cost / (price - supply_cost))
                if price > supply_cost
                else expected_attendees
            )
...
if __name__ == "__main__":
    ...
    for key, info in planner.EVENT_TYPES.items():
        print(
            f"  {info['emoji']} {info['name']} ({info['typical_size']}, {info['duration']})"
        )

    # Create events
    e1 = planner.create_event(
    ...
    budget = planner.calculate_budget(
        e1["id"], ticket_price=25, venue_cost=50, supply_cost=5, expected_attendees=15
    )
    ...
    for e in upcoming:
        ...
        print(
            f"  {info.get('emoji', '📅')} {e['name']} - {e['date']} ({e['attending']}/{e['capacity']})"
        )
