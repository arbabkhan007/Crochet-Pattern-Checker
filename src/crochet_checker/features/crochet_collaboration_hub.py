...
class CollaborationHub:
    ...
    def add_version(
        self, project_id: str, content: str, author: str, comment: str = ""
    ):
...
"""
        for version in history["versions"]:
            report += (
                f"  • {version['author']} - {version['comment'] or 'No comment'}\n"
            )
...
if __name__ == "__main__":
    ...
    print("\n📝 Adding versions...")
    hub.add_version(
        project_id, "Row 1: Ch 20, sc across", "alice_designer", "Initial pattern"
    )
    hub.add_version(
        project_id,
        "Row 1: Ch 25, sc across\nRow 2: Ch 1, turn, sc across",
        "bob_tester",
        "Added row 2",
    )
