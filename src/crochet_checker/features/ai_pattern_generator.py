...
class AIPatternGenerator:
    ...
    def generate_pattern(
        ...
        if difficulty == "beginner":
            ...
            stitches = (
                self.STITCH_DATABASE["basic"] + self.STITCH_DATABASE["intermediate"][:3]
            )
        else:
            stitches = (
                self.STITCH_DATABASE["basic"]
                + self.STITCH_DATABASE["intermediate"]
                + self.STITCH_DATABASE["advanced"][:2]
            )
        ...
        pattern = {
            ...
            "pattern": self._generate_instructions(
                project_type, dimensions, stitches, difficulty
            ),
    ...
    def _generate_instructions(
        self, project_type: str, dimensions: dict, stitches: list[str], difficulty: str
    ) -> list[str]:
        ...
        if project_type == "scarf":
            ...
            instructions.append(
                f"Row 1: {stitches[1]} in 2nd ch from hook and each ch across. Turn."
            )

            if difficulty == "beginner":
                instructions.append(
                    f"Rows 2-{dimensions['length'] * 4}: Ch 1, {stitches[1]} in each st across. Turn."
                )
            elif difficulty == "intermediate":
                instructions.append(
                ...
                if len(stitches) > 2:
                    instructions.append(
                        f"Row 21: Ch 1, {stitches[2]} in each st across. Turn."
                    )
                    instructions.append("Rows 22-40: Repeat Row 21.")
            else:
                instructions.append(
                    "Row 1: Ch 3, *skip 2 sts, dc in next st, ch 2, dc in same st; repeat from * across."
                )
            ...
            instructions.append(
                f"Round 2: Ch 1, 2 {stitches[1]} in each st around. (12 sts)"
            )
            instructions.append(
                f"Round 3: Ch 1, *{stitches[1]}, inc; repeat from * around. (18 sts)"
            )
            instructions.append(
                "Continue increasing 6 sts every round until circumference matches head size."
            )
    ...
    def estimate_time(self, pattern: dict) -> dict:
        ...
        multiplier = difficulty_multiplier.get(
            pattern.get("difficulty", "beginner"), 1.0
        )
