...
class PatternCodeGenerator:
    ...
    def generate_granny_square(self, rounds: int = 4) -> dict:
        ...
        for r in range(2, rounds + 1):
            ...
            pattern.append(
                "  *3 dc, ch 2, 3 dc, ch 2, 3 dc, ch 2, 3 dc, ch 1* in each corner"
            )
            pattern.append("  Repeat around, sl st to top of ch-3")
            stitch_count += 8 * (r - 1)
    ...
    def generate_ripple_blanket(self, width_sts: int = 100, rows: int = 50) -> dict:
        ...
        for row in range(1, rows + 1):
            if row == 1:
                pattern.append("Row 1: Dc in 4th ch from hook and each ch across")
            else:
    ...
    def generate_amigurumi_sphere(self, final_size: int = 30) -> dict:
        ...
        while current_count < final_size:
            if round_num == 1:
                ...
                pattern.append(
                    f"Round {round_num}: *sc {inc_every - 1}, inc* around ({current_count + 6})"
                )
                current_count += 6
            round_num += 1
        ...
        while current_count > 6:
            ...
            pattern.append(
                f"Round {round_num}: *sc {dec_every - 1}, dec* around ({current_count - 6})"
            )
            current_count -= 6
            round_num += 1
