...
class GiftPlanner:
    ...
    GIFT_IDEAS = [
        ...
        GiftIdea(
            "Baby Blanket", "baby", "Intermediate", 15, 25, tags=["baby", "shower"]
        ),
        GiftIdea(
            "Cozy Scarf", "wearable", "Beginner", 6, 12, tags=["winter", "unisex"]
        ),
        GiftIdea(
            "Granny Square Bag",
            "accessory",
            ...
            8,
            15,
            tags=["fashion", "colorful"],
        ),
        GiftIdea(
            "Dishcloth Set", "home", "Beginner", 3, 5, tags=["kitchen", "practical"]
        ),
        GiftIdea("Beanie Hat", "wearable", "Beginner", 3, 8, tags=["winter", "quick"]),
        GiftIdea(
            "Lovie/Comforter", "baby", "Advanced Beginner", 5, 12, tags=["baby", "gift"]
        ),
        GiftIdea(
            "Phone Cozy", "accessory", "Beginner", 2, 4, tags=["quick", "practical"]
        ),
        GiftIdea(
            "Table Runner",
            "home",
            ...
            10,
            20,
            tags=["home_decor", "elegant"],
        ),
        GiftIdea(
            "Stuffed Heart", "toy", "Beginner", 2, 3, tags=["valentine", "romantic"]
        ),
        GiftIdea(
            "Market Bag", "accessory", "Intermediate", 6, 10, tags=["eco", "practical"]
        ),
                ...
                try:
                    ...
                    if occasion_date < now:
                        occasion_date = datetime(
                            now.year + 1, info["month"], info["day"]
                        )
    ...
    def calculate_work_plan(self, gift_id: str, hours_per_day: float = 1.0) -> dict:
        ...
        return {
            ...
            "suggested_daily_hours": round(
                min(hours_per_day, hours_per_day_needed * 1.2), 1
            ),
    ...
    def display_dashboard(self):
        ...
        if upcoming:
            ...
            for occ in upcoming[:5]:
                print(
                    f"  {occ['emoji']} {occ['occasion']:20s} {occ['days_away']} days ({occ['date']})"
                )
        ...
        if urgent:
            ...
            for g in urgent:
                print(
                    f"  {g['urgency']} {g['item']} for {g['recipient']} - {g['days_left']} days left"
                )
        ...
        for idea in quick_gifts[:5]:
            print(
                f"  • {idea.name} ({idea.difficulty}, {idea.estimated_hours}h, ${idea.estimated_cost})"
            )
...
if __name__ == "__main__":
    ...
    print("✅ Added 2 recipients")

    # Plan gifts
    g1 = planner.plan_gift(
        "Mom",
        "Birthday",
    ...
    for g in urgent:
        print(
            f"  {g['urgency']} {g['item']} for {g['recipient']} - {g['days_left']} days"
        )
