...
def diff_patterns(pattern1: str, pattern2: str) -> DiffResult:
    ...
    unchanged = sum(
        1 for line in difflib.unified_diff(lines1, lines2) if line.startswith(" ")
    )
    ...
    if similarity == 1.0:
        ...
        summary = (
            f"⚠️  Patterns have moderate differences ({similarity * 100:.1f}% match)"
        )
    else:
        summary = (
            f"❌ Patterns are significantly different ({similarity * 100:.1f}% match)"
        )
