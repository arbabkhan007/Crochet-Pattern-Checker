...
from _pdf_utils import FONT, setup_unicode_font
...
class PremiumPatternPDF:
    ...
    def create_instructions_page(
        self, instructions: list[str], section_title: str = "Instructions"
    ):
        self.pdf.add_page()
    ...
    def create_back_page(
        self, designer: str = "", website: str = "", copyright_text: str = ""
    ):
        self.pdf.add_page()
        r, g, b = self._hex_to_rgb(self.theme["primary"])
        self.pdf.set_fill_color(r, g, b)
    ...
    def generate_full_pattern_pdf(self, pattern_data: dict, output_path: str) -> str:
        ...
        if pattern_data.get("materials"):
            self.create_materials_page(
                materials=pattern_data["materials"], gauge=pattern_data.get("gauge", "")
            )
        ...
        if pattern_data.get("notes") or pattern_data.get("tips"):
            self.create_notes_page(
                notes=pattern_data.get("notes", []), tips=pattern_data.get("tips", [])
            )
...
if __name__ == "__main__":
    ...
    pattern = {
        ...
        "notes": [
            "This pattern uses basic stitches only.",
            "Adjust hook size to match gauge.",
        ],
        "tips": [
            "Use stitch markers every 10 stitches",
    ...
    print("  Pages: 5 (Cover + Materials + Instructions + Notes + Back)")
