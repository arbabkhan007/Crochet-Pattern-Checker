...
class PatternTesterManager:
    ...
    SKILL_LEVELS = {
        ...
        "intermediate": {
            "weight": 2,
            "desc": "Comfortable with patterns, multiple stitch types",
        },
    ...
    def assign_test(self, pattern_name: str, tester_id: str, due_days: int = 14) -> str:
    ...
    def submit_feedback(self, test_id: str, feedback: dict) -> dict:
        ...
        if tester:
            ...
            quality = (
                sum(
                    feedback.get(k, {}).get("rating", 0)
                    for k in ["clarity", "accuracy", "difficulty_match", "enjoyment"]
                )
                / 4
            )
    ...
    def find_best_testers(
        self, skill_level: str = None, specialty: str = None, max_active_tests: int = 2
    ) -> list[dict]:
        ...
        for tester in self.testers.values():
            if not tester.active:
                continue
            if (
                tester.id in active_counts
                and active_counts[tester.id] >= max_active_tests
            ):
                continue
    ...
    def get_testing_dashboard(self) -> dict:
        ...
        overdue = [
            t
            for t in all_tests
            if t.status in ("assigned", "in_progress") and t.due_date < today
        ]
        ...
        recent_feedback = [
            {
                ...
                "tester": self.testers.get(t.tester_id, Tester("", "")).name,
                "status": t.status,
        ...
        return {
            ...
            "overdue": [
                {"id": t.id, "pattern": t.pattern_name, "due": t.due_date}
                for t in overdue
            ],
...
if __name__ == "__main__":
    ...
    t2 = mgr.add_tester(
        "Bob", "bob@example.com", "advanced", specialties=["garments", "colorwork"]
    )
    t3 = mgr.add_tester(
        "Carol", "carol@example.com", "beginner", specialties=["blankets", "home"]
    )
    print("✅ Added 3 testers")
    ...
    print("✅ Submitted feedback for test 1")
    ...
    for c in candidates:
        print(
            f"  • {c['name']} ({c['skill_level']}, {c['tests_completed']} tests done)"
        )
