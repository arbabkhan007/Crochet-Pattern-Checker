...
class WholesaleCalculator:
    ...
    def calculate_price(
        ...
        return {
            ...
            "profit_margin_pct": round(profit / rounded_price * 100, 1)
            if rounded_price > 0
            else 0,
            "model": model,
        }
    ...
    def calculate_bulk_order(
        ...
        total_cost = self.calculate_cost(total_materials, total_hours, 12.0, 0.10)[
            "total_cost"
        ]
    ...
    def craft_fair_planner(
        self, items: list[dict], booth_fee: float = 50, expected_visitors: int = 200
    ) -> dict:
        ...
        total_cost = sum(
            self.calculate_cost(item.get("materials", 0), item.get("hours", 0))[
                "total_cost"
            ]
            * item.get("quantity", 0)
            for item in items
        ...
        return {
            "inventory": {
                ...
                "total_cost": round(total_cost, 2),
                "items_by_type": {
                    item.get("name", "Unknown"): item.get("quantity", 0)
                    for item in items
                },
            },
            "costs": {
            ...
            "projections": {
                ...
                "estimated_profit": round(
                    estimated_revenue - total_cost - booth_fee, 2
                ),
            },
...
class ShippingCalculator:
    ...
    def calculate_shipping(
        self,
        item_type: str = "amigurumi_medium",
        quantity: int = 1,
        domestic: bool = True,
    ...
    def suggest_free_shipping_threshold(
        self,
        avg_item_price: float,
        avg_shipping: float,
        profit_margin_target: float = 0.30,
    ) -> dict:
    ...
    def compare_carriers(
        self, item_type: str, weight_lbs: float, domestic: bool = True
    ) -> list[dict]:
        ...
        carriers = [
            ...
            },
            {
                ...
                "cost": round(base * 2.0, 2),
                "days": "1-2",
                "tracking": True,
            },
            {
                "name": "UPS Ground",
                "cost": round(base * 1.1, 2),
                "days": "3-5",
                "tracking": True,
            },
            {
                "name": "FedEx Ground",
                "cost": round(base * 1.15, 2),
                "days": "3-5",
                "tracking": True,
            },
        ]
        ...
        if not domestic:
            carriers = [
                ...
                {
                    "name": "USPS Priority Intl",
                    "cost": round(base * 1.5, 2),
                    "days": "6-10",
                    "tracking": True,
                },
                {
                    "name": "UPS Worldwide",
                    "cost": round(base * 2.5, 2),
                    "days": "3-5",
                    "tracking": True,
                },
            ]
...
if __name__ == "__main__":
    ...
    print("  Item: Amigurumi Bunny")
    print(f"  Materials: ${price['cost_breakdown']['materials']}")
    print(
        f"  Labor: ${price['cost_breakdown']['labor']} ({price['cost_breakdown']['hours']}h)"
    )
    ...
    print(
        f"  Inventory: {fair['inventory']['total_items']} items (${fair['inventory']['total_value']})"
    )
