...
class AdvancedTextureGenerator:
    def __init__(self):
        self.texture_types = {
            ...
            'bobble': {'complexity': 'intermediate', 'technique': 'multiple_stitches_one'},
            'popcorn': {'complexity': 'intermediate', 'technique': 'cluster_work'},
            'puff': {'complexity': 'intermediate', 'technique': 'yarn_overs'},
            'ripple': {'complexity': 'advanced', 'technique': 'increase_decrease_pattern'},
            'waffle': {'complexity': 'advanced', 'technique': 'fpdc_bpdc_pattern'},
            'basketweave': {'complexity': 'advanced', 'technique': 'alternating_fpdc_bpdc'}
        }
    ...
    def _generate_cable_pattern(self, dimensions: dict) -> list:
        ...
        pattern.append(f"Row 1: dc in 4th ch from hook, dc across")
        pattern.append(f"Row 2: ch 3, *skip 2, dc in next 2, dc in each of skipped 2 (cross), dc across, repeat from *")
        pattern.append(f"Row 3: ch 3, dc across")
        pattern.append(f"Row 4: ch 3, dc across")
        pattern.append(f"Repeat rows 2-4 for cable pattern")
        return pattern
    
    def _generate_bobble_pattern(self, dimensions: dict) -> list:
        pattern = []
        pattern.append(f"Ch multiple of 6 + 3")
        pattern.append(f"Row 1: sc in 2nd ch from hook, sc across")
        pattern.append(f"Row 2: ch 1, *sc 3, bobble (yo, insert hook, yo, pull up loop) 5 times in next st, yo, pull through all 6 loops, sc 3, repeat from * across")
        pattern.append(f"Row 3: ch 1, sc across, working bobble in sc below")
        pattern.append(f"Repeat rows 2-3")
        return pattern
    
    def _generate_waffle_pattern(self, dimensions: dict) -> list:
        pattern = []
        pattern.append(f"Ch multiple of 3 + 2")
        pattern.append(f"Row 1: dc in 3rd ch from hook, dc across")
        pattern.append(f"Row 2: ch 2, *fpdc 2, bpdc 1, repeat from * across, turn")
        pattern.append(f"Row 3: ch 2, *bpdc 1, fpdc 2, repeat from * across, turn")
        pattern.append(f"Repeat rows 2-3 for waffle texture")
        return pattern
    
    def _generate_basic_texture(self, texture_type: str, dimensions: dict) -> list:
        pattern = []
        pattern.append(f"Ch multiple of 4 + 2")
        pattern.append(f"Row 1: sc in 2nd ch from hook, sc across")
        
        if texture_type == 'popcorn':
            pattern.append(f"Row 2: ch 1, *sc 2, popcorn (5 dc in next st, remove hook, insert in 1st dc, pull loop through), sc 2, repeat from *")
        elif texture_type == 'puff':
            pattern.append(f"Row 2: ch 1, *puff (yo, insert hook, yo, pull up loop) 3 times in same st, yo, pull through all loops, ch 1, repeat from *")
