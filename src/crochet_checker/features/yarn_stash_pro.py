...
class YarnStashManagerPro:
    ...
    def assign_to_project(self, yarn_id: str, project_name: str) -> dict:
        ...
        if project_name not in [p["name"] for p in self.data["projects"]]:
            ...
            project = next(
                p for p in self.data["projects"] if p["name"] == project_name
            )
    ...
    def get_stash_stats(self) -> dict:
        ...
        smallest = sorted(
            [y for y in yarns if y["yards"] > 0 and (y["yards"] - y["used"]) < 50],
            key=lambda y: y["yards"] - y["used"],
        )[:5]
        ...
        return {
            ...
            "smallest_skeins": [
                {"name": y["name"], "remaining": round(y["yards"] - y["used"], 1)}
                for y in smallest
            ],
        }
    ...
    def find_yarn(
        ...
        if weight:
            results = [
                y
                for y in results
                if y["weight"] and weight.lower() in y["weight"].lower()
            ]
        if color:
            results = [
                y for y in results if y["color"] and color.lower() in y["color"].lower()
            ]
        if fiber:
            results = [
                y for y in results if y["fiber"] and fiber.lower() in y["fiber"].lower()
            ]
        if min_yards:
    ...
    def add_to_shopping_list(
        self, name: str, weight: str = "", yards_needed: float = 0, notes: str = ""
    ) -> dict:
    ...
    def get_shopping_list(self) -> list[dict]:
        """Get shopping list"""
        return [
            item for item in self.data["shopping_list"] if not item.get("purchased")
        ]
...
if __name__ == "__main__":
    ...
    # Add yarns
    stash.add_yarn(
        "Super Saver Red", "Red Heart", "Red", "Worsted", 364, 198, "Acrylic", cost=4.99
    )
    stash.add_yarn(
        "Merino Grey",
        "Malabrigo",
