...
class AIPatternOptimizer:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if "yarn_efficiency" in goals:
                if "dc dc dc" in line_lower:
                    ...
                    optimizations.append(f"Row {i}: Replaced 3 dc with tr for efficiency")
            
            # Optimize for time efficiency
            if "time_efficiency" in goals:
                if "sc sc sc sc sc" in line_lower:
                    optimizations.append(f"Row {i}: Consider using longer stitches for speed")
    ...
    def calculate_improvement_score(self, original: str, optimized: str) -> dict:
        ...
        return {
            ...
            "efficiency_score": min(100, (original_lines / optimized_lines) * 100) if optimized_lines > 0 else 100,
            "improvement_percent": ((original_lines - optimized_lines) / original_lines * 100) if original_lines > 0 else 0
        }
...
Row 3: 10 hdc"""
    ...
    improvement = optimizer.calculate_improvement_score(pattern, result['optimized_pattern'])
