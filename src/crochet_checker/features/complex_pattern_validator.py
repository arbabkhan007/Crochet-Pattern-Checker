...
class ComplexPatternValidator:
    ...
    def validate_complex_pattern(self, pattern_text: str, pattern_metadata: dict) -> dict:
        ...
        if 'repeat_multiple' in pattern_metadata:
            if not self._check_pattern_repeat(lines, pattern_metadata['repeat_multiple']):
                warnings.append({
                    'type': 'pattern_repeat',
        ...
        if self._has_special_stitches(pattern_text) and not self._has_definitions(pattern_text):
            warnings.append({
    ...
    def _has_definitions(self, pattern_text: str) -> bool:
        ...
        return 'abbreviation' in pattern_text.lower() or 'special stitch' in pattern_text.lower() or 'definition' in pattern_text.lower()
