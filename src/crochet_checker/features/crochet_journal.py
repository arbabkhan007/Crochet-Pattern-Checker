...
class CrochetJournal:
    ...
    def get_mood_analysis(self) -> dict:
        ...
        # Most common mood
        top_mood = (
            max(mood_counts.items(), key=lambda x: x[1]) if mood_counts else ("none", 0)
        )
        ...
        for day, moods in mood_by_day.items():
            positive = sum(
                ...
                if k in ("happy", "calm", "inspired", "relaxed")
            )
            total = sum(moods.values())
            day_moods[day] = positive / max(1, total) * 100

        best_day = (
            max(day_moods.items(), key=lambda x: x[1]) if day_moods else ("none", 0)
        )
    ...
    def get_weekly_summary(self, weeks_back: int = 0) -> dict:
        ...
        return {
            ...
            "projects": projects,
            "avg_energy": round(
                sum(e.energy for e in week_entries) / len(week_entries), 1
            ),
        }
...
"""
        ...
        if mood.get("mood_distribution"):
            report += "\n  Mood Breakdown:\n"
            for mood_name, count in sorted(
                mood["mood_distribution"].items(), key=lambda x: -x[1]
            ):
...
if __name__ == "__main__":
    ...
    journal.add_entry(
        ...
        minutes=45,
        rounds=5,
        project="Bunny Amigurumi",
        notes="Finally got the ears right!",
    ...
    journal.add_entry(
        ...
        minutes=30,
        rounds=3,
        project="Granny Blanket",
        notes="Mindless crochet while watching TV",
    )
    ...
    journal.add_entry(
        ...
        rounds=1,
        project="Bunny Amigurumi",
        notes="Kept messing up round 7, had to frog twice",
    )
    ...
    journal.add_entry(
        ...
        rounds=8,
        project="New Design",
        notes="Started sketching a new fox pattern!",
    )
    ...
    journal.add_entry(
        ...
        rounds=4,
        project="Granny Blanket",
        notes="Perfect evening crochet session",
    )
