...
class YarnCalculator:
    ...
    def _round_yarn(self, round_obj):
        return sum(
            STITCH_YARN_USAGE.get(op.stitch_type.value, 1.5) * op.count
            for inst in round_obj.instructions
            for op in inst.operations
        )


def estimate_yarn(
    pattern, yarn_weight="worsted", grams_per_skein=100, yards_per_skein=200
):
    return YarnCalculator(yarn_weight, grams_per_skein, yards_per_skein).estimate(
        pattern
    )
