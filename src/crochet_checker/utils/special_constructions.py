...
class GrannySquareValidator:
    ...
    def validate(
        self, rounds_data: list[dict], setup_round: int = 3
        ...
        for i in range(1, len(sorted_rounds)):
            ...
            if "sl st" in instr and "join" in instr:
                analysis.warnings.append(
                    f"Round {curr['round']}: Join detected in spiral"
                )
            if "turn" in instr:
                analysis.warnings.append(
                    f"Round {curr['round']}: Turn detected in spiral"
                )
...
if __name__ == "__main__":
    ...
    from crochet_checker.utils.enhanced_pdf import AestheticPrinter
