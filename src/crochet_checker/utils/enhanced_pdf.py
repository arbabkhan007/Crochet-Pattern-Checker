...
class AestheticPrinter:
    ...
    def header(self, title: str, subtitle: str = ""):
        ...
        print(
            f"{self.bg_h}{self.B}{self.p}║{' ' * max(0, padding)}{title}{' ' * max(0, width - padding - len(title))}║{self.R}"
        )
        if subtitle:
            padding2 = (width - len(subtitle)) // 2
            print(
                f"{self.bg_h}{self.D}{self.s}║{' ' * max(0, padding2)}{subtitle}{' ' * max(0, width - padding2 - len(subtitle))}║{self.R}"
            )
    ...
    def round_item(
        self, round_num: int, instruction: str, count: int, expected: int = None
    ):
        ...
        if expected is not None:
            ...
            print(
                f"  {marker} {self.p}R{round_num:2d}{self.R}: {self.t}{instruction:30s}{self.R} ({self.B}{count}{self.R})"
            )
        else:
            print(
                f"    {self.p}R{round_num:2d}{self.R}: {self.t}{instruction:30s}{self.R} ({self.B}{count}{self.R})"
            )
    ...
    def piece_card(
        ...
        print(
            f"  {self.bg_s}│{self.R}   {self.s}Quantity:{self.R}     {self.t}{quantity}{self.R}"
        )
        print(
            f"  {self.bg_s}│{self.R}   {self.s}Rounds:{self.R}       {self.t}{rounds}{self.R}"
        )
        print(
            f"  {self.bg_s}│{self.R}   {self.s}Final Count:{self.R}  {self.t}{final_count}{self.R}"
        )
        if construction:
            print(
                f"  {self.bg_s}│{self.R}   {self.s}Construction:{self.R} {self.a}{construction}{self.R}"
            )
        print(f"  {self.bg_s}{'└' + '─' * width + '┘'}{self.R}")
...
class EnhancedPDFExtractor:
    ...
    def _detect_sections(self):
        ...
        for line in lines:
            ...
            piece_match = re.match(
                r"^(\d+)\s*[·:]\s*(.+?)(?:\s*[-–—].*)?$", line_stripped
            )
            if piece_match:
                num = piece_match.group(1)
                name = piece_match.group(2).strip()
                if len(name) < 50 and not any(
                    c in name for c in ["sc", "dc", "inc", "dec"]
                ):
                    if current_section:
                        self.sections.append(
                            PatternSection(
        ...
        for section in self.sections:
            if section.section_type == "instructions":
                ...
                if rounds:
                    ...
                    qty_match = re.search(
                        r"make\s*(\d+)", section.content, re.IGNORECASE
                    )
    ...
    def _extract_rounds(self, content: str) -> list[dict]:
        ...
        for line in lines:
            ...
            if not line_stripped:
                continue
            if re.match(
                r"^\s*R(?:oun)?d\s+Instruction\s+Sts", line_stripped, re.IGNORECASE
            ):
                continue
            ...
            if table_match:
                ...
                if (
                    any(t in instr.lower() for t in crochet_terms)
                    and 1 <= rn <= 200
                    and 0 < cnt <= 1000
                ):
                    rounds.append(
            ...
            range_match = re.search(
                r"(\d+)[–-](\d+)\s+(.+?)\s*\((\d+)\s*rnds?\)\s*(\d+)",
                line_stripped,
                re.IGNORECASE,
            )
            if range_match and not any(
                r["round"] == int(range_match.group(1)) for r in rounds
            ):
                for rnd in range(
                    ...
                    rounds.append(
                        {
                            "round": rnd,
                            "instruction": range_match.group(3).strip(),
                            "count": int(range_match.group(5)),
                            "raw": line_stripped,
                            "is_range": True,
                        }
                    )
    ...
    def _build_structured_data(self, pieces: list[dict]) -> dict:
        ...
        construction = []
        tl = full_text.lower()
        if "spiral" in tl or "continuous" in tl:
            construction.append("spiral")
        if "joined" in tl or "sl st to close" in tl:
            construction.append("joined_rounds")
        if "granny square" in tl or "granny-square" in tl:
            construction.append("granny_square")
        if "magic ring" in tl:
            construction.append("magic_ring")

        diff = "Unknown"
        if "advanced beginner" in tl:
            diff = "Advanced Beginner"
        elif "beginner" in tl:
            diff = "Beginner"
    ...
    def generate_aesthetic_report(self, result: dict):
        ...
        for piece in pieces:
            ...
            pp.piece_card(
                ...
                quantity=qty,
                final_count=total,
                construction=sd.get("construction_type", [""])[0]
                if sd.get("construction_type")
                else "",
            )
            ...
            completeness = min(
                100,
                (len(rounds) / max(piece.get("total_rounds", len(rounds)), 1)) * 100,
            )
...
if __name__ == "__main__":
    ...
    if pdf_path:
        ...
        pp.piece_card(
            "Head", rounds=15, quantity=1, final_count=6, construction="spiral"
        )
        print()
