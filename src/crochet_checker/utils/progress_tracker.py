...
class ProgressTracker:
    def __init__(self, pattern, project_name=None):
        ...
        self.progress = {
            i: ProgressEntry(round_number=i) for i in range(1, self.total_rounds + 1)
        }
        self.notes = []
    ...
    def complete_round(self, round_number, notes=None):
        if round_number in self.progress:
            ...
            if notes:
                self.progress[round_number].notes = notes
            return True
        return False
    ...
    def save(self, filepath):
        data = {
            "pattern_name": self.project_name,
            "total_rounds": self.total_rounds,
            "progress": {
                str(k): {"completed": v.completed, "notes": v.notes}
