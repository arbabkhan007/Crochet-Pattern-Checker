...
class Unroller:
    ...
    def __init__(self):
        self.bracket_pattern = re.compile(
            r"\*([^*]+)\*(?:\s*(?:repeat|rep)\s*(\d+)?\s*times?|\s*around)?",
            re.IGNORECASE,
        )
        self.multiplier_pattern = re.compile(r"(\d+)\s*times?", re.IGNORECASE)
        self.paren_pattern = re.compile(r"\(([^()]+)\)")

    def unroll(self, instruction_text: str, repeat_count: int | None = None) -> str:
    ...
    def _unroll_single_level(self, text: str, default_repeat: int | None = None) -> str:
    ...
    def parse_repeat_tree(self, instruction_text: str) -> list[RepeatNode]:
        ...
        for match in self.bracket_pattern.finditer(instruction_text):
            ...
            nodes.append(RepeatNode(count=count, content=content, children=children))

        return nodes

    def validate_corners(
        self, instruction_text: str, canvas_queue: list[str]
    ) -> list[str]:
        ...
        for i, expected in enumerate(instruction_corners):
            ...
            if i >= len(canvas_corners):
                errors.append(
                    f"Expected ch-{expected_size} sp at position {i}, but canvas ended"
                )
    ...
    def calculate_stitch_count(
        self, instruction_text: str, glossary: dict = None
    ) -> int:
...
class RecursiveLoopUnroller:
    ...
    def __init__(self):
        ...
        self._multiplier = (
            r"\s*(?:repeat\s*(\d+)\s*times?|(\d+)\s*times?|(twice)|(thrice))?"
        )
        self.repeat_pattern = re.compile(
            r"\*\s*(.*?)\s*\*" + self._multiplier, re.IGNORECASE | re.DOTALL
        )
        self.bracket_pattern = re.compile(
            r"\[\s*(.*?)\s*\]" + self._multiplier, re.IGNORECASE | re.DOTALL
        )
    ...
    def _unroll_recursive(
        self, text: str, operations: list[AtomicOperation], depth: int
    ) -> None:
    ...
    def _parse_atomics(
        ...
        for segment in segments:
            ...
            times_match = re.search(
                r"(\d+)\s*times?|twice|thrice", segment, re.IGNORECASE
            )
            multiplier = 1
            if times_match:
    ...
    def validate_unrolling(
        self, original: str, operations: list[AtomicOperation]
    ) -> list[str]:
