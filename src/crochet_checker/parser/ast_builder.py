...
class StitchInstruction:
    ...
    stitch_type: str
    count: int = 1
    target: str | None = None  # e.g., "next st", "ch-sp"
    line_number: int = 0
...
class ASTBuilder:
    ...
    def __init__(self):
        self.lexer = Lexer()
        self.stitch_pattern = re.compile(
            r"(\d+)\s*(sc|dc|hdc|tr|sl\s*st|ch|skip|inc|dec)", re.IGNORECASE
        )
                ...
                for i, line in enumerate(lines):
                    ...
                    if self._is_piece_header(cleaned):
                        ...
                        current_piece = PieceNode(
                            name=piece_name, construction_mode=construction_mode
                        )
    ...
    def _is_piece_header(self, line: str) -> bool:
        ...
        return bool(
            re.search(r"(piece|module|part|section)\s+\d+", line, re.IGNORECASE)
        )

    def _extract_piece_name(self, line: str) -> str:
        """Extract piece name from header"""
        match = re.search(
            r"(piece|module|part|section)\s+(\d+|\w+)", line, re.IGNORECASE
        )
        if match:
    ...
    def _parse_round(self, line: str, round_num: int, line_num: int) -> RoundNode:
        ...
        round_node = RoundNode(number=round_num, raw_text=line, line_number=line_num)
...
"""
    ...
    for piece in ast.pieces:
        ...
        for r in piece.rounds:
            print(
                f"  Round {r.number}: {len(r.instructions)} instructions, count={r.stitch_count}"
            )
...
class MultiPieceASTBuilder:
    ...
    def __init__(self):
        ...
        self.piece_numbered_pattern = re.compile(rf"(?i)^{self._piece_keywords}\s+\d+")
        self.piece_bare_pattern = re.compile(rf"(?i)^{self._piece_keywords}\s*$")
    ...
    def build(self, markdown_text: str) -> PatternNode:
        ...
        for line_num, raw_line in enumerate(markdown_text.split("\n"), 1):
            ...
            if re.match(
                r"(?i)^#*\s*(glossary|abbreviations|stitch\s+definitions)\s*$", line
            ):
            ...
            if self._is_piece_header(line):
                ...
                current_piece.metadata["yarn_pointer"] = (
                    0  # yarn pointer resets per piece
                )
    ...
    def _is_piece_header(self, line: str) -> bool:
        ...
        return bool(
            self.piece_numbered_pattern.match(line)
            or self.piece_bare_pattern.match(line)
        )
    ...
    def _detect_construction_mode(self, line: str) -> ConstructionMode | None:
    ...
    def _parse_round(self, line: str, round_num: int, line_num: int) -> RoundNode:
        ...
        for match in self.stitch_pattern.finditer(line):
            round_node.instructions.append(
                StitchInstruction(
                    stitch_type=match.group(2).lower(),
                    count=int(match.group(1)),
                    line_number=line_num,
    ...
    def validate_piece_boundaries(self, pattern: PatternNode) -> list[str]:
        ...
        for piece in pattern.pieces:
            ...
            if piece.construction_mode == ConstructionMode.ROUNDS and any(
                "row" in r.raw_text.lower() for r in piece.rounds
            ):
                errors.append(
