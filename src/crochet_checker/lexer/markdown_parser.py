...
from ..ast.nodes import ConstructionType, PatternNode, PieceNode, RoundNode
...
class MarkdownParser:
    ...
    def __init__(self):
        ...
        self.construction_pattern = re.compile(
            r"(?:worked\s+in\s+)?(rounds?|rows?)", re.IGNORECASE
        )
    ...
    def parse(self, markdown_text: str) -> PatternNode:
        ...
        if not pattern.pieces:
            default_piece = PieceNode(
                name="Main", construction_type=ConstructionType.ROUNDS
            )
        ...
        for line in lines:
            ...
            if header_match:
                ...
                if level == 1:
                    ...
                    kw in title.lower() for kw in ["glossary", "abbreviation", "terms"]
                ):
                    current_type = "glossary"
                elif any(
                    kw in title.lower() for kw in ["note", "instruction", "special"]
                ):
                    current_type = "notes"
                else:
                    current_type = "piece"
    ...
    def _parse_rounds(self, text: str, start_line: int) -> list[RoundNode]:
        ...
        for i, line in enumerate(lines, start_line):
            ...
            if round_match:
                ...
                round_node = RoundNode(number=round_num, line_number=i, raw_text=line)
