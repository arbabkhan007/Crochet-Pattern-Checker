...
class AIProvider:
    ...
    def explain_pattern(self, pattern, report, context=""):
        ...
        if self.config.provider == "openai":
            return self._openai(pattern, report, context)
        if self.config.provider == "anthropic":
            return self._anthropic(pattern, report, context)
        if self.config.provider == "gemini":
            return self._gemini(pattern, report, context)
        if self.config.provider == "ollama":
            return self._ollama(pattern, report, context)
        return self._rule_explain(pattern, report)
    ...
    def _prompt(self, p, r, ctx):
        ...
        if r.errors:
            prompt += f"\nErrors ({len(r.errors)}):\n" + "\n".join(
                f"  - {e.message}" for e in r.errors[:3]
            )
        if r.warnings:
            prompt += f"\nWarnings ({len(r.warnings)}):\n" + "\n".join(
                f"  - {w.message}" for w in r.warnings[:3]
            )
            ...
            return (
                c.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a crochet pattern expert.",
                        },
                        {"role": "user", "content": self._prompt(p, r, ctx)},
                    ],
        ...
        try:
            ...
            return (
                c.messages.create(
                    ...
                    max_tokens=self.config.max_tokens,
                    messages=[{"role": "user", "content": self._prompt(p, r, ctx)}],
                )
                .content[0]
                .text
            )
    ...
    def _gemini(self, p, r, ctx):
        try:
            ...
            return (
                genai.GenerativeModel(self.config.model or "gemini-pro")
                .generate_content(self._prompt(p, r, ctx))
                .text
            )
        except Exception as e:
