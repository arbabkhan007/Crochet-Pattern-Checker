...
from ..visualization.measurements import (
    StitchDimensions,
    measure_pattern,
)
from .mesh import (
...
class SurfaceSimulator:
    ...
    def analyze_shape(self, pattern):
        ...
        if not pattern.rounds:
            a.explanation = "Pattern uses rows not rounds."
            return a
        counts, prev = [], 0
        ...
        if a.has_increases and not a.has_decreases:
            ...
            a.detected_shape = DetectedShape.CONE
            a.confidence = 0.6
            a.explanation = "Only decreases - cone."
        else:
            if a.constant_rounds >= len(counts) * 0.7:
                a.detected_shape = DetectedShape.TUBE
                a.confidence = 0.8
                a.explanation = "Constant count - tube."
            else:
                a.detected_shape = DetectedShape.UNKNOWN
                a.confidence = 0.3
    ...
    def generate_mesh(self, pattern):
        ...
        m = measure_pattern(pattern, self.dims)
        radius = m.max_radius_mm / 10
        height = m.total_height_mm / 10
        if radius <= 0:
            radius = 5.0
        if height <= 0:
            height = 5.0
        seg = 32
        s = a.detected_shape
        if s == DetectedShape.SPHERE:
            return generate_sphere_mesh(radius, seg, seg // 2)
        elif s == DetectedShape.HAT:
            return generate_hat_mesh(
                radius, radius * 1.3, height * 0.7, height * 0.05, seg
            )
        elif s == DetectedShape.FLAT_CIRCLE:
            return generate_flat_circle_mesh(radius, seg)
        elif s == DetectedShape.TUBE:
            return generate_tube_mesh(radius, radius, height, seg)
        elif s == DetectedShape.CONE:
            return generate_tube_mesh(radius, radius * 0.3, height, seg)
        elif s == DetectedShape.BOWL:
            return generate_sphere_mesh(radius, seg, seg // 4)
        else:
            return generate_sphere_mesh(radius, seg)
