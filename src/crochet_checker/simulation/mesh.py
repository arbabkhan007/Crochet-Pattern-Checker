...
class Mesh(BaseModel):
    ...
    def add_vertex(self, x, y, z):
        self.vertices.append(Vec3(x=x, y=y, z=z))
    ...
    def to_obj(self):
        lines = ["# Crochet Pattern Checker 3D Mesh", f"# Object: {self.name}", ""]
        for v in self.vertices:
    ...
    def save_obj(self, filepath):
        ...
        Path(filepath).write_text(self.to_obj())
...
def generate_sphere_mesh(radius, segments=24, rings=12, center=None):
    ...
    for j in range(1, rings):
        ...
        ry = center.y + radius * math.cos(phi)
        cr = []
        for i in range(segments):
            theta = (2 * math.pi * i) / segments
            cr.append(
                mesh.add_vertex(
                    center.x + rr * math.cos(theta), ry, center.z + rr * math.sin(theta)
                )
            )
...
def generate_tube_mesh(bottom_radius, top_radius, height, segments=24, center=None):
    ...
    for i in range(segments):
        ...
        bi.append(
            mesh.add_vertex(
                center.x + bottom_radius * math.cos(a),
                center.y,
                center.z + bottom_radius * math.sin(a),
            )
        )
    ti = []
    for i in range(segments):
        ...
        ti.append(
            mesh.add_vertex(
                center.x + top_radius * math.cos(a),
                center.y + height,
                center.z + top_radius * math.sin(a),
            )
        )
    for i in range(segments):
        ni = (i + 1) % segments
        mesh.add_quad(bi[i], bi[ni], ti[ni], ti[i])
    bc = mesh.add_vertex(center.x, center.y, center.z)
    for i in range(segments):
        mesh.add_face(bc, bi[(i + 1) % segments], bi[i])
    tc = mesh.add_vertex(center.x, center.y + height, center.z)
    for i in range(segments):
        mesh.add_face(tc, ti[i], ti[(i + 1) % segments])
    return mesh
...
def generate_flat_circle_mesh(radius, segments=36, center=None):
    ...
    for i in range(segments):
        mesh.add_face(ci, ri[i], ri[(i + 1) % segments])
    return mesh


def generate_hat_mesh(
    crown_radius, brim_radius, crown_height, brim_height=2.0, segments=36, center=None
):
    if center is None:
        center = Vec3()
    mesh = Mesh(name="hat")
    ...
    for i in range(segments):
        ...
        ir.append(
            mesh.add_vertex(
                center.x + crown_radius * math.cos(a),
                by,
                center.z + crown_radius * math.sin(a),
            )
        )
        orr.append(
            mesh.add_vertex(
                center.x + brim_radius * math.cos(a),
                by,
                center.z + brim_radius * math.sin(a),
            )
        )
    for i in range(segments):
        ni = (i + 1) % segments
        mesh.add_quad(ir[i], ir[ni], orr[ni], orr[i])
    return mesh
