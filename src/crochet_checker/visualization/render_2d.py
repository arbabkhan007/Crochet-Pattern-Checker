...
class Render2D:
    ...
    def render(self, pattern: Pattern, validation_report=None) -> str:
        ...
        elements.append(
            f'<rect width="{self.width}" height="{total_height}" fill="white"/>'
        )
        ...
        diagram_svg = generate_circle_diagram(
            pattern, width=self.width, height=self.height - 60
        )
        # Extract inner SVG content (skip the outer svg tags)
        inner = "\n".join(diagram_svg.split("\n")[2:-1])  # Skip svg open/close
        elements.append('<g transform="translate(0, 60)">')
        elements.append(inner)
        elements.append("</g>")

        # Stitch count chart (bottom)
        chart_y = self.height + 20
        chart_svg = generate_stitch_count_chart(
            pattern, width=self.width - 40, height=250
        )
        ...
        if validation_report:
            ...
            elements.append(
                self._render_validation_summary(validation_report, summary_y)
            )
