...
class LoopState:
    ...
    def work(self, round_number: int, cluster_id: str | None = None) -> bool:
        """Mark loop as worked"""
...
class StitchConsumer:
    ...
    def add_loop(self, round_number: int, position: int) -> LoopState:
        ...
        loop = LoopState(loop_id=loop_id, round_number=round_number, position=position)
    ...
    def get_loop_at(self, round_number: int, position: int) -> LoopState | None:
    ...
    def get_unworked_loops(self, round_number: int | None = None) -> list[LoopState]:
    ...
    def check_exhaustion(self, round_number: int, allow_unworked: bool = False) -> dict:
        ...
        result = {
            "round": round_number,
            "total_loops": len(
                [l for l in self.loops.values() if l.round_number == round_number]
            ),
    ...
    def get_round_summary(self, round_number: int) -> dict:
        ...
        return {
            ...
            "worked": state_counts.get("worked_into", 0)
            + state_counts.get("consumed_in_cluster", 0),
...
if __name__ == "__main__":
    ...
    print("Consumed 3 loops in cluster")
