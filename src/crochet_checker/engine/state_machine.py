...
class StateMachine:
    ...
    def initialize_round(
        self, stitch_count: int, round_number: int = 1, create_mode: bool = False
    ):
    ...
    def execute_instruction(self, instruction: InstructionNode, *legacy_args) -> int:
        ...
        try:
            ...
            self.errors.append(f"Line {instruction.line_number}: {e!s}")
    ...
    def validate_directional_movement(self, instruction: InstructionNode) -> bool:
        ...
        if instruction.target_position is not None:
            ...
            for i in range(self.hook.position, target):
                if (
                    i < len(self.canvas)
                    and self.canvas[i].status == LoopStatus.AVAILABLE
                ):
                    self.warnings.append(
                        f"[WARN] Line {instruction.line_number}: "
...
if __name__ == "__main__":
    ...
    instr1 = InstructionNode(
        action="stitch", stitch_type="magic ring", count=6, line_number=1
    )
