...
class GlobalAnchorGraph:
    ...
    def get_nodes_by_position(
        ...
        return [
            node
            for node in self.get_nodes_in_round(round_number)
    ...
    def find_anchor_in_previous_rounds(
        self, position: int, anchor_type: AnchorType, max_depth: int = -1
    ) -> list[AnchorNode]:
        """
