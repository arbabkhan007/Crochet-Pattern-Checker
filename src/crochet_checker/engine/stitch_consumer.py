...
from .canvas_queue import CanvasElement, CanvasQueue, OrphanedStitchError, StitchStatus
...
class ConsumptionResult:
    ...
    new_element: CanvasElement | None = None
...
class StitchConsumer:
    ...
    def consume_single(
        self, canvas: CanvasQueue, stitch_type: str
    ) -> ConsumptionResult:
        ...
        return ConsumptionResult(
            consumed_count=1, produced_count=1, elements_consumed=[element]
        )

    def consume_cluster(
        self, canvas: CanvasQueue, cluster_size: int, stitch_type: str = "dc"
    ) -> ConsumptionResult:
        ...
        for _ in range(cluster_size):
            ...
            if not element:
                raise ValueError(
                    f"Not enough elements for cluster (need {cluster_size})"
                )
    ...
    def consume_decrease(
        self, canvas: CanvasQueue, decrease_count: int
    ) -> ConsumptionResult:
        ...
        for _ in range(decrease_count):
            ...
            if not element:
                raise ValueError(
                    f"Not enough elements for decrease (need {decrease_count})"
                )
    ...
    def consume_increase(
        self, canvas: CanvasQueue, increase_count: int
    ) -> ConsumptionResult:
    ...
    def skip_stitches(self, canvas: CanvasQueue, skip_count: int) -> ConsumptionResult:
        ...
        return ConsumptionResult(
            consumed_count=skip_count, produced_count=0, elements_consumed=skipped
        )

    def consume_space(
        self, canvas: CanvasQueue, space_type: str, stitches_in_space: int
    ) -> ConsumptionResult:
        ...
        if stitches_in_space > element.capacity:
            raise ValueError(
                f"Space capacity is {element.capacity}, trying to work {stitches_in_space}"
            )
    ...
    def check_orphan_stitches(
        self, canvas: CanvasQueue, has_explicit_unworked: bool = False
    ) -> dict:
    ...
    def parse_and_consume(
        self, canvas: CanvasQueue, instruction: str
    ) -> ConsumptionResult:
        ...
        # Check for increase
        inc_match = re.search(
            r"(\d+)\s*sc\s+in\s+next\s+st", instruction, re.IGNORECASE
        )
        ...
        space_match = re.search(
            r"(\d+)\s*([a-z]+)\s+in\s+ch-(\d+)-sp", instruction, re.IGNORECASE
        )
        if space_match:
            stitch_count = int(space_match.group(1))
            space_size = int(space_match.group(3))
            return self.consume_space(canvas, "ch_sp", stitch_count)
