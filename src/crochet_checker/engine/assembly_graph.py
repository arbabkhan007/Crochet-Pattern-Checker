...
class AssemblyGraph:
    ...
    def _find_anchor(self, piece_name: str, point_id: str) -> AnchorPoint | None:
    ...
    def validate_flower_assembly(
        self, petal_count: int, center_piece: str = None
    ) -> bool:
        """Validate a flower assembly with petals around a center"""
        # Find all petal pieces
        petals = [name for name in self.pieces.keys() if "petal" in name.lower()]

        if len(petals) != petal_count:
            self.errors.append(f"Expected {petal_count} petals, found {len(petals)}")
            return False
