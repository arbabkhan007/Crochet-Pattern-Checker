...
try:
    ...
    _spec = importlib.util.spec_from_file_location(
        "canvas_queue",
        Path(__file__).resolve().parents[1] / "engine" / "canvas_queue.py",
    )
...
class OrphanedStitchAnalyzer:
    ...
    def add_stitch(self, round_number: int, position: int) -> CanvasStitch:
        ...
        stitch = CanvasStitch(
            stitch_id=stitch_id, round_number=round_number, position=position
        )
    ...
    def get_stitch(self, stitch_id: str) -> CanvasStitch | None:
        """Get a stitch by ID"""
    ...
    def add_skip_directive(
        self, round_number: int, positions: list[int], directive: str
    ) -> None:
        """Add a directive to skip specific positions"""
        self.skip_directives.append(
            {"round": round_number, "positions": positions, "directive": directive}
        )
    ...
    def _get_stitch_at_position(
        self, round_number: int, position: int
    ) -> CanvasStitch | None:
    ...
    def analyze_exhaustion(
        self, round_number: int, raise_on_orphans: bool = True
    ) -> ExhaustionReport:
        """
        ...
        if len(skipped) > len(stitches) * 0.5:
            warnings.append(
                f"More than 50% of stitches were skipped ({len(skipped)}/{len(stitches)})"
            )
    ...
    def analyze_all_rounds(
        self, raise_on_orphans: bool = False
    ) -> dict[int, ExhaustionReport]:
        """Analyze exhaustion for all rounds"""
        reports = {}
        for round_number in sorted(self.round_stitches.keys()):
            try:
                reports[round_number] = self.analyze_exhaustion(
                    round_number, raise_on_orphans
                )
    ...
    def validate_pattern(self) -> list[str]:
        ...
        for round_number in sorted(self.round_stitches.keys()):
            try:
                ...
                errors.append(f"Round {round_number}: {e!s}")
...
if __name__ == "__main__":
    ...
    print(
        f"   Worked: {report.worked_stitches}, Intentionally unworked: {len(report.intentionally_unworked)}"
    )
