...
class TurningChain:
    ...
    round_number: int
    line_number: int
    stitch_type: Optional[str] = None  # What it counts as (e.g., "dc")
...
class TurningChainRulesEngine:
    ...
    def parse_turning_chain(self, instruction: str, round_number: int, 
                           line_number: int) -> Optional[TurningChain]:
        """
        ...
        # Check if it explicitly says "counts as"
            ...
            counts_match = re.search(r'counts?\s+as\s+(\w+)', instruction, re.IGNORECASE)
    ...
    def allocate_chain(self, chain: TurningChain) -> ChainAllocation:
        ...
        allocation = ChainAllocation(
            chain=chain,
            stitch_count_addition=addition,
            reason=reason
        )
    ...
    def calculate_round_stitch_count(self, round_number: int, 
                                    base_count: int) -> Dict:
        """
        ...
        """
        round_chains = [c for c in self.turning_chains if c.round_number == round_number]
    ...
    def validate_turning_chains(self, round_number: int) -> List[str]:
        ...
        for chain in round_chains:
            ...
            if chain.counts_as_stitch and chain.stitch_type:
                expected_count = {
                    "sc": 1, "hdc": 2, "dc": 3, "tr": 4, "dtr": 5
                }.get(chain.stitch_type)
...
if __name__ == "__main__":
    ...
    print(f"Base: {result1['base_count']}, Addition: {result1['turning_chain_addition']}, Total: {result1['total_count']}")
    ...
    print(f"Base: {result2['base_count']}, Addition: {result2['turning_chain_addition']}, Total: {result2['total_count']}")
