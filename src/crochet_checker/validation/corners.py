...
class CornerSequenceValidator:
    ...
    def add_corner_space(self, round_number: int, corner_index: int, 
                        chain_count: int, position: int) -> CornerSpace:
        """Add a corner space"""
        space = CornerSpace(
    ...
    def add_repeat_block(self, expected_chain_count: int, corner_index: int,
                        line_number: int) -> RepeatBlock:
    ...
    def parse_corner_from_instruction(self, instruction: str) -> Optional[int]:
        """Extract chain count from instruction like 'ch-2 sp' or 'ch 3 space'"""
        match = re.search(r'ch[\s-]*(\d+)[\s-]*(?:sp|space)', instruction, re.IGNORECASE)
        if match:
