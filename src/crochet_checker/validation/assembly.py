...
class AssemblyGraphValidator:
    ...
    def validate_assembly(self) -> dict:
        ...
        hub_pieces = {
            pid for pid in core_participants if pid not in neighbor_participants
        }
    ...
    def get_assembly_summary(self) -> dict:
        ...
        return {
            ...
            "adjacency": {
                pid: sorted(list(adj)) for pid, adj in self.adjacency.items()
            },
        }
...
if __name__ == "__main__":
    ...
    print(
        f"   Neighbor joins: {result['neighbor_joins']}/{result['expected_neighbor_joins']}"
    )
