...
def check(pattern_file, strict, output_json, verbose):
    ...
    if not output_json:
        ...
        for i, r in enumerate(items):
            ...
            sc = (
                r.computed_stitch_count
                if i == 0
                else r.compute_stitch_count_with_context(prev)
            )
            dc = sc
            for inst in r.instructions:
                if inst.stated_stitch_count is not None:
                    dc = inst.stated_stitch_count
                    break
            console.print(
                f"  {chr(8220) + 'Round' if hasattr(r, 'round_number') else 'Row'} {num}: {dc} stitches"
            )
            prev = dc
        console.print()
...
def render(pattern_file, output):
    ...
    (out_dir / "diagram.svg").write_text(generate_circle_diagram(pattern))
    console.print("  OK diagram.svg")
    (out_dir / "stitch_counts.svg").write_text(generate_stitch_count_chart(pattern))
    console.print("  OK stitch_counts.svg")
    (out_dir / "crochet_chart.svg").write_text(generate_crochet_chart(pattern))
    ...
    console.print(
        f"  Max radius: {m.max_radius_mm:.1f} mm ({m.max_radius_inches:.2f} in)"
    )
    console.print(
        f"  Max diameter: {m.max_radius_mm * 2:.1f} mm ({m.max_diameter_inches:.2f} in)"
    )
    console.print(
...
def measure(pattern_file):
    ...
    console.print(
        f"  Max circumference: {m.max_circumference_mm:.1f} mm ({m.max_circumference_inches:.2f} in)"
    )
    console.print(
        f"  Total height: {m.total_height_mm:.1f} mm ({m.total_height_inches:.2f} in)"
    )
...
def _display(report, verbose=False):
    ...
    t.append(status, style=f"bold {c}")
    ...
    if report.row_transitions:
        ...
        table.add_row(
            "Transitions",
            Text("OK" if e == 0 else "warn", style="green" if e == 0 else "yellow"),
            f"{e} errors",
        )
    console.print(table)
    if report.errors:
        ...
        for f in report.errors:
            console.print(f"  [red]X[/red] [{f.location}] {f.message}")
    if report.warnings:
...
def render_3d(pattern_file, output):
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    console.print(
        f"Shape: [cyan]{analysis.detected_shape.value}[/cyan] ({analysis.confidence:.0%})"
    )
...
def explain_cmd(pattern_file, ai_provider):
    ...
    if suggestions:
        ...
        for s in suggestions:
            console.print(f"  [{s.confidence}] {s.error_location}: {s.suggestion}")
...
def image_cmd(pattern_file, output, provider, style):
    ...
    category = getattr(pattern.metadata, "category", "") or ""
    img = ImageProvider(
        ImageConfig(provider=provider, style=style)
    ).generate_cover_image(title, category)
    if output is None:
        output = str(
            Path("output")
            / Path(pattern_file).stem
...
def yarn_calc(pattern_file, weight, grams, yards):
    ...
    pattern = CrochetParser().parse(text)
    est = estimate_yarn(
        pattern, yarn_weight=weight, grams_per_skein=grams, yards_per_skein=yards
    )
    console.print(
    ...
    console.print("\u2501" * 30)
    console.print(
        f"Total yarn: [cyan]{est.total_yards:.1f} yards[/cyan] ({est.total_meters:.2f} meters)"
    )
    if est.total_grams:
        console.print(f"Weight: [cyan]{est.total_grams:.1f} grams[/cyan]")
    console.print(
...
def progress(pattern_file, complete, uncomplete, note, save):
    ...
    if complete and tracker.complete_round(complete):
        console.print(f"[green]\u2713 Round {complete} complete[/green]")
    if uncomplete and tracker.uncomplete_round(uncomplete):
        console.print(f"[yellow]\u25cb Round {uncomplete} incomplete[/yellow]")
    if note:
        tracker.add_note(note)
        console.print("[blue]\U0001f4dd Note added[/blue]")
    if save:
        tracker.save(save)
        console.print(f"[green]\U0001f4be Saved to {save}[/green]")
