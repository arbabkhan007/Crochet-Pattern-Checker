...
class PatternExplainer:
    ...
    def _summary(self, p, r, m):
        ...
        v = (
            "passes all checks"
            if "PASS" in s and "WARNING" not in s
            else "passes with minor warnings"
            if "PASS" in s
            else "needs review"
            if "REVIEW" in s
            else "has errors"
    ...
    def _explanation(self, p, r, m):
        lines = [
            f"**Construction:** Worked in {p.construction.value.replace(chr(95), chr(32))}s."
        ]
        items = p.rounds or p.rows
        if items and len(items) >= 2:
            ...
            if fc == 0:
                for i in items[0].instructions:
                    if i.stated_stitch_count:
                        fc = i.stated_stitch_count
                        break
            lc = items[-1].computed_stitch_count
            if lc == 0:
                for i in items[-1].instructions:
                    if i.stated_stitch_count:
                        lc = i.stated_stitch_count
                        break
            if fc > 0 and lc > 0:
                if lc > fc:
                    lines.append(f"**Growth:** {fc} to {lc} stitches - increasing.")
                elif lc < fc:
                    lines.append(f"**Shrinking:** {fc} to {lc} stitches - decreasing.")
                else:
                    lines.append(f"**Steady:** stays at {fc} stitches.")
        if r.errors:
            lines.append(f"\n**Errors ({len(r.errors)}):**")
            for e in r.errors[:3]:
    ...
    def _highlights(self, p, r, m):
        ...
        items = p.rounds or p.rows
        if not items:
            return ["No rounds detected"]
        fs = items[0].computed_stitch_count
        for i in items[0].instructions:
            if i.stated_stitch_count:
                fs = i.stated_stitch_count
                break
        ls = items[-1].computed_stitch_count
        for i in items[-1].instructions:
            if i.stated_stitch_count:
                ls = i.stated_stitch_count
                break
        h.append(f"Starts with {fs} stitches")
        h.append(f"Ends with {ls} stitches")
        h.append(
    ...
    def _recommendations(self, p, r, m):
        recs = []
        if r.errors:
            recs.append("Fix errors before crocheting")
        if r.warnings:
            recs.append("Review warnings")
        items = p.rounds or p.rows
        if items:
            if not p.gauge:
                recs.append("Add gauge for consistent sizing")
            if not p.yarn or not p.yarn.weight:
                recs.append("Add yarn weight info")
            if any("magic ring" in r2.source_text.lower() for r2 in items[:2]):
                recs.append("Uses magic ring!")
    ...
    def _guess_shape(self, p, m):
        ...
        if not items:
            return "unknown"
        fc = items[0].computed_stitch_count or 6
        lc = items[-1].computed_stitch_count or 6
        has_dec = False
        if len(items) >= 3:
            for r in items[-3:]:
                for inst in r.instructions:
                    for op in inst.operations:
                        if op.stitch_type.value in ("decrease", "sc2tog"):
                            has_dec = True
        if has_dec and lc <= fc + 2:
            return "sphere or amigurumi"
        elif lc > fc and not has_dec:
            return "flat circle or cone"
        elif lc < fc:
            return "decreasing shape"
        else:
            return "tube or straight piece"
    ...
    def _difficulty(self, p):
        ...
        for r in items:
            for inst in r.instructions:
                for op in inst.operations:
                    if op.stitch_type.value in ("treble", "double_treble"):
                        has_complex = True
                if inst.repeat_count and inst.repeat_count > 1:
                    has_repeats = True
        parts = []
        if n <= 5:
            parts.append("Short - beginner-friendly")
        elif n <= 15:
            parts.append("Medium-length")
        else:
            parts.append("Long pattern")
        if has_complex:
            parts.append("advanced stitches")
        if has_repeats:
            parts.append("has repeats")
        return ". ".join(parts) + "."
