    ...
    def _suggest(self, p, err):
        ...
        if "stitch count" in msg or "mismatch" in msg or "inconsistent" in msg:
            return Suggestion(
                ...
                confidence="medium",
                explanation="Count carefully: sc=1, inc=2, dec=1",
            )
        if "missing" in msg:
            return Suggestion(
                error_location=err.location,
                error_message=err.message,
                suggestion="Add stitch count in parentheses at end of round",
                confidence="high",
            )
        if "unknown" in msg:
            return Suggestion(
                error_location=err.location,
                error_message=err.message,
                suggestion="Check abbreviation: sc, hdc, dc, tr, sl st, ch",
                confidence="medium",
            )
        return Suggestion(
            error_location=err.location,
            error_message=err.message,
            suggestion="Review this round manually",
            confidence="low",
        )
