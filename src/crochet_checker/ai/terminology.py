...
class TerminologyTranslator:
    def translate_us_to_uk(self, text):
        return self._translate(text, US_TO_UK, "US", "UK")

    def translate_uk_to_us(self, text):
        return self._translate(text, UK_TO_US, "UK", "US")

    def translate(self, text, f, t):
        f, t = f.upper(), t.upper()
        if f == "US" and t == "UK":
            return self.translate_us_to_uk(text)
        elif f == "UK" and t == "US":
            return self.translate_uk_to_us(text)
        ...
        def repl(match):
            ...
            for term in sorted_terms:
                if re.match(re.escape(term), orig, re.IGNORECASE):
                    rep = mapping[term]
                    tr = self._case(orig, rep)
                    changes.append(f"{orig} -> {tr}")
                    return tr
            return orig
