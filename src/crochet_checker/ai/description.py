...
class DescriptionGenerator:
    def generate(self, pattern, report=None):
        ...
        return PatternDescription(
            title=self._title(pattern, m),
            short_description=self._short(pattern, m),
            full_description=self._full(pattern, m, report),
            materials_list=self._materials(pattern),
            skill_level=self._skill(pattern, m),
            finished_size=self._size(m),
            tags=self._tags(pattern, m),
        ...
        for r in check:
            for inst in r.instructions:
                for op in inst.operations:
                    if op.stitch_type.value in vals:
                        return True
        return False

    def _title(self, p, m):
        t = p.metadata.title or "Crochet Pattern"
    ...
    def _materials(self, p):
        mats = []
        if p.yarn and p.yarn.name:
            mats.append(f"Yarn: {p.yarn.name}")
        if p.hook and p.hook.size_mm:
            mats.append(f"Hook: {p.hook.size_mm}mm")
        if not mats:
            mats = ["Yarn (see pattern)", "Hook (see pattern)"]
    ...
    def _skill(self, p, m):
        ...
        if not items:
            return "Beginner"
        n = len(items)
        hc = self._has_st(items, ["treble", "double_treble"])
        hi = self._has_st(items, ["increase"])
        hd = self._has_st(items, ["decrease", "sc2tog"])
        if n <= 5 and not hc:
            return "Beginner"
        if n <= 15 and not hc:
            return "Easy" if hi and hd else "Beginner-Easy"
        if hc:
            return "Intermediate"
