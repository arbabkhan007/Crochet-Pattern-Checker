...
class KnowledgeBase:
        ...
        tmpl["structures"].append(
            {
                ...
                "has_decreases": any(
                    "dec" in r.get("instruction", "").lower() for r in rounds
                ),
            }
        )
                ...
                suggestions.append(
                    {
                        ...
                        "suggestion": ep_data["fix_suggestions"][0]
                        if ep_data["fix_suggestions"]
    ...
    def get_learning_stats(self) -> dict:
        ...
        return {
            ...
            "yarn_entries": sum(
                len(v) for v in self.knowledge["yarn_knowledge"].values()
            ),
    ...
    def _extract_features(self, pattern_data: dict) -> dict:
        ...
        features = {
            ...
            "has_special": any(
                s in instructions for s in ["popcorn", "bobble", "cable", "shell"]
            ),
            "unique_instructions": len(
                set(r.get("instruction", "").lower() for r in rounds)
            ),
        }
        return features
...
class ValidatorAgent(BaseAgent):
    ...
    def execute(self, task: AgentTask) -> dict:
        ...
        if not rounds:
            ...
            result["errors"].append(
                {"type": "no_rounds", "message": "No rounds found in pattern"}
            )
            return result
    ...
    def _calculate_count(self, instruction: str, prev_count: int) -> int:
        ...
        if prev_count == 0:
            ...
            match = re.search(
                r"(\d+)\s*(?:sc|dc|hdc)\s*(?:in|into)\s*(?:MR|magic)", instruction
            )
            if match:
...
class PDFGeneratorAgent(BaseAgent):
    ...
    def _generate_pattern_document(
        self, title: str, pattern: dict, rounds: list[dict], style: str
    ) -> str:
...
"""
        # Style banner
        if style == "elegant":
            doc += "*A handcrafted crochet pattern*\n\n"
            doc += "---\n\n"
        elif style == "modern":
...
class WebSearchAgent(BaseAgent):
    ...
    def _search_yarn(self, query: str) -> list[dict]:
        ...
        return [
            {
                "name": "DK Weight Guide",
                "info": "DK (#3) is 12-17 WPI, best with 3.5-4.5mm hook",
                "source": "Yarn Guide",
            },
            {
                "name": "Worsted Weight Guide",
                "info": "Worsted (#4) is 9-12 WPI, best with 4.5-5.5mm hook",
                "source": "Yarn Guide",
            },
            {
                "name": "Cotton vs Acrylic",
                "info": "Cotton: durable, breathable. Acrylic: soft, affordable.",
                "source": "Materials Guide",
            },
        ]
    ...
    def _get_trends(self) -> list[dict]:
        ...
        return [
            {
                "trend": "Amigurumi Food",
                "popularity": 95,
                "description": "Miniature crocheted food items",
            },
            {
                "trend": "Granny Square Modern",
                "popularity": 88,
                "description": "Modern colorways of classic granny squares",
            },
            {
                "trend": "Sustainable Crochet",
                "popularity": 82,
                "description": "Eco-friendly yarn and zero-waste patterns",
            },
            {
                "trend": "Wearable Crochet",
                "popularity": 75,
                "description": "Tops, cardigans, and accessories",
            },
            {
                "trend": "3D Crochet Art",
                "popularity": 70,
                "description": "Sculptural and artistic crochet pieces",
            },
        ]
...
class PatternGeneratorAgent(BaseAgent):
    ...
    def _generate_pattern(self, description: str, ptype: str, size: str) -> dict:
        ...
        if (
            ptype == "sphere"
            or "ball" in description.lower()
            or "amigurumi" in description.lower()
        ):
            return self._gen_sphere(size_multiplier, description)
        elif (
            ptype == "cylinder"
            or "hat" in description.lower()
            or "cup" in description.lower()
        ):
            return self._gen_cylinder(size_multiplier, description)
        elif (
            ptype == "flat"
            or "coaster" in description.lower()
            or "blanket" in description.lower()
        ):
    ...
    def _gen_sphere(self, mult: float, desc: str) -> dict:
        ...
        for i in range(even_rounds):
            rn = max_width + 1 + i
            rounds.append(
                {"round": rn, "instruction": "sc in each st around", "count": count}
            )
    ...
    def _gen_cylinder(self, mult: float, desc: str) -> dict:
        ...
        for i in range(2, 5):
            count = 6 * i
            rounds.append(
                {"round": i, "instruction": f"[{i - 2} sc, inc] × 6", "count": count}
            )
        ...
        for i in range(3, 0, -1):
            ...
            rounds.append(
                {
                    "round": rn,
                    "instruction": f"[{i - 2} sc, dec] × 6" if i > 1 else "dec × 6",
                    "count": count,
                }
            )
    ...
    def _gen_flat(self, mult: float, desc: str) -> dict:
        ...
        rounds.append(
            {
                "round": 1,
                "instruction": "Ch 4, sl st to join. Ch 3, 2 dc in ring, [ch 2, 3 dc] × 3, ch 2",
                "count": 12,
            }
        )
        rounds.append(
            {
                "round": 2,
                "instruction": "Sl st to corner. Ch 3, 2 dc, ch 2, 3 dc. (3 dc, ch 2, 3 dc) in each corner",
                "count": 24,
            }
        )
        ...
        for i in range(3, total_rounds + 1):
            ...
            rounds.append(
                {
                    "round": i,
                    "instruction": "Sl st to corner. Ch 3, (2 dc, ch 2, 3 dc) in corner. 3 dc in each side space, (3 dc, ch 2, 3 dc) in each corner",
                    "count": count,
                }
            )
...
class AnalystAgent(BaseAgent):
    ...
    def execute(self, task: AgentTask) -> dict:
        ...
        if "dec" in all_instr:
            complexity += 10
        if "cable" in all_instr:
            complexity += 25
        if "popcorn" in all_instr or "bobble" in all_instr:
            complexity += 20
        if "shell" in all_instr:
            complexity += 15
        if len(rounds) > 20:
            complexity += 10
        if len(rounds) > 50:
        ...
        result["estimated_time_minutes"] = round(
            (total_stitches * time_per_stitch) / 60, 0
        )
        ...
        # Skill factors
        if "cable" in all_instr:
            result["skill_factors"].append("Cable work")
        if "dec" in all_instr and "inc" in all_instr:
            result["skill_factors"].append("Shaping")
        if "magic ring" in all_instr or "mr" in all_instr:
            result["skill_factors"].append("Magic ring")
        if "ch" in all_instr and "space" in all_instr:
            result["skill_factors"].append("Chain spaces")

        # Predictions based on knowledge
        stats = self.knowledge.get_learning_stats()
        if stats["patterns_analyzed"] > 10:
            result["predictions"].append(
                f"Based on {stats['patterns_analyzed']} analyzed patterns, this is a {'simple' if complexity < 30 else 'moderate' if complexity < 60 else 'complex'} project"
            )
...
class LearningAgent(BaseAgent):
    ...
    def _evolve_knowledge(self) -> dict:
        ...
        for level, features_list in self.knowledge.knowledge[
            "difficulty_indicators"
        ].items():
            if len(features_list) >= 5:
                improvements.append(
                    f"Difficulty prediction for '{level}' now based on {len(features_list)} examples"
                )

        # Check stitch pattern confidence
        high_confidence = sum(
            1
            for sp in self.knowledge.knowledge["stitch_patterns"].values()
            if sp["occurrences"] >= 5
        )
        improvements.append(
            f"{high_confidence} stitch patterns have high confidence (5+ occurrences)"
        )

        # Error pattern detection improvement
        common_errors = sum(
            1
            for ep in self.knowledge.knowledge["error_patterns"].values()
            if ep["count"] >= 3
        )
        improvements.append(
            f"{common_errors} error patterns are well-documented (3+ occurrences)"
        )

        return {"evolved": True, "improvements": improvements, "stats": stats}
    ...
    def _generate_insights(self) -> list[dict]:
        ...
        if stats["patterns_analyzed"] == 0:
            insights.append(
                {
                    "type": "info",
                    "message": "No patterns analyzed yet. Start validating patterns!",
                }
            )
            return insights
...
class Orchestrator:
    ...
    def run_pipeline(self, pattern: dict, pipeline: str = "full") -> dict:
        ...
        if pipeline == "full":
            ...
            if v.get("valid"):
                print(
                    f"  {C.MINT}   ✅ Pattern valid! Score: {v.get('score', 0)}/100{C.R}"
                )
            else:
            ...
            a = results["analysis"]
            print(
                f"  {C.TEAL}   📊 Complexity: {a.get('complexity_score', 0)}/100{C.R}"
            )
            print(
                f"  {C.TEAL}   ⏱️  Est. time: {a.get('estimated_time_minutes', 0)} min{C.R}"
            )
            ...
            print(
                f"  {C.PEACH}   📚 Patterns analyzed: {stats.get('patterns_analyzed', 0)}{C.R}"
            )
            print(
                f"  {C.PEACH}   🧠 Stitch patterns learned: {stats.get('stitch_patterns_learned', 0)}{C.R}"
            )
            ...
            print(
                f"  {C.SKY}   📄 Generated {results['pdf'].get('pages', 0)} pages{C.R}"
            )
    ...
    def get_agent_status(self) -> dict:
        ...
        return {
            agent.agent_type.value: agent.get_stats() for agent in self.agents.values()
        }
    ...
    def print_dashboard(self):
        ...
        for agent_type, stats in self.get_agent_status().items():
            ...
            print(
                f"  {color}{icon} {stats['name']:20s}{C.R} Tasks: {completed:3d} | Avg: {stats['avg_time']:.2f}s"
            )
...
╚══════════════════════════════════════════════════════════╝{C.R}
    ...
    search_results = orch.run_pipeline(
        {"query": "amigurumi animals", "search_type": "patterns"}, pipeline="search"
    )
