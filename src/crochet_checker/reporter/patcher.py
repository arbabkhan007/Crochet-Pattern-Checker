...
class DiffPatchGenerator:
    ...
    def analyze_round(self, round_number: int, instructions: List[str],
                     expected_count: int, line_numbers: List[int] = None) -> PatchReport:
        """
        ...
        if actual_count != expected_count:
            patch = self._generate_count_fix_patch(
                round_number, line_numbers[0] if line_numbers else 1,
    ...
    def _count_stitches(self, instructions: List[str]) -> Tuple[int, List[Dict]]:
        ...
        for instruction in instructions:
            ...
            for match in re.finditer(r'(\d+)\s+(sc|dc|hdc|tr|dtr)', instruction, re.IGNORECASE):
                n = int(match.group(1))
                total += n
                count += n
            ...
            for match in re.finditer(r'(sc|dc|hdc|tr|dtr)\s+(\d+)', instruction, re.IGNORECASE):
                n = int(match.group(2))
                total += n
                count += n
            ...
            residue = re.sub(r'\d+\s+(?:sc|dc|hdc|tr|dtr)', ' ', instruction, flags=re.IGNORECASE)
            residue = re.sub(r'(?:sc|dc|hdc|tr|dtr)\s+\d+', ' ', residue, flags=re.IGNORECASE)
            single_stitches = re.findall(r'\b(sc|dc|hdc|tr|dtr)\b', residue, re.IGNORECASE)
    ...
    def _generate_count_fix_patch(self, round_number: int, line_number: int,
                                 instruction: str, actual: int, expected: int) -> Optional[Patch]:
    ...
    def _suggest_add_stitches(self, instruction: str, count: int) -> Optional[str]:
        ...
        if match:
            ...
            return instruction[:match.start(1)] + str(new_count) + instruction[match.end(1):]
    ...
    def _suggest_remove_stitches(self, instruction: str, count: int) -> Optional[str]:
    ...
    def _check_turning_chain(self, round_number: int, instruction: str,
                            line_number: int) -> Optional[Patch]:
        """Check for turning chain issues"""
        # Find turning chain
        ...
        if expected_chain and chain_count != expected_chain:
            ...
            corrected = instruction[:match.start(1)] + str(expected_chain) + instruction[match.end(1):]
    ...
    def _check_corner_sequence(self, round_number: int, instructions: List[str],
                              line_numbers: List[int]) -> Optional[Patch]:
        """Check for corner sequence issues"""
        ...
        for i, instruction in enumerate(instructions):
            ...
            if matches and ('corner' in instruction.lower() or 
                           'sp' in instruction.lower() or 
                           'space' in instruction.lower()):
        ...
        if len(set(corner_counts)) > 1:
            ...
            patch = Patch(
                round_number=round_number,
                line_number=line_numbers[corner_positions[0]] if corner_positions else 1,
                original_text=instructions[corner_positions[0]] if corner_positions else "",
