...
class UnsupportedSyntaxFallback:
    ...
    def detect_unsupported(
        self, text: str, line_number: int = 0
    ) -> list[tuple[str, str]]:
        """
        ...
        for pattern in self.unsupported_patterns:
            ...
            if match:
                ...
                if "backward" in pattern or "through" in pattern or "behind" in pattern:
                    ...
                    "few" in pattern
                    or "several" in pattern
                    or "some" in pattern
                    or "evenly" in pattern
                    or "needed" in pattern
                    or "appropriate" in pattern
                ):
    ...
    def parse_with_fallback(
        self, text: str, line_number: int = 0, expected_count: int | None = None
    ) -> CountResult:
        """
        ...
        if count is not None:
            return CountResult(
                count=count, trust_level=TrustLevel.TRUSTED, original_text=text
            )
    ...
    def _try_normal_parse(self, text: str) -> int | None:
    ...
    def _fallback_unknown_count(self, text: str, expected: int | None) -> CountResult:
    ...
    def _fallback_spatial_reference(
    ...
    def _fallback_vague_instruction(
        self, text: str, expected: int | None
    ) -> CountResult:
        ...
        return CountResult(
            ...
            suggestions=["Specify exact number of stitches", "Provide explicit count"],
        )
...
if __name__ == "__main__":
    ...
    result1 = fallback.parse_with_fallback(
        "Round 3: 2 dc in each st around (24 sts)", 3
    )
    ...
    print("\nTest 3: Vague instruction")
    result3 = fallback.parse_with_fallback("sc in several stitches around", 7)
