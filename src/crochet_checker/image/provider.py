...
class ImageConfig(BaseModel):
    provider: Literal["placeholder", "dalle", "gemini", "stable_diffusion"] = (
        "placeholder"
    )
...
class ImageProvider:
    ...
    def _get_api_key(self):
        keys = {
            ...
            "stable_diffusion": "REPLICATE_API_TOKEN",
        }
    ...
    def generate_cover_image(self, title, category=""):
        ...
        if self.config.provider == "dalle":
            return self._dalle(title, category)
        if self.config.provider == "gemini":
            return self._gemini(title, category)
        if self.config.provider == "stable_diffusion":
            return self._sd(title, category)
        return self._placeholder(title, category)
...
<text x="200" y="360" text-anchor="middle" font-family="Georgia" font-size="16" fill="hsl({h1},40%,30%)">{title[:30]}</text>
    ...
    def _icon(self, cat):
        ...
        if "hat" in c:
            return '<path d="M-40,-30 Q0,-60 40,-30 L40,20 Q0,30 -40,20 Z" fill="hsl(200,60%,50%)"/>'
        if "scarf" in c:
            return '<rect x="-60" y="-10" width="120" height="20" rx="5" fill="hsl(0,60%,60%)"/>'
        if "amigurumi" in c or "sphere" in c:
            return '<circle r="50" fill="hsl(120,60%,60%)"/><circle cx="-15" cy="-15" r="8" fill="white"/><circle cx="15" cy="-15" r="8" fill="white"/>'
        if "blanket" in c:
            return '<rect x="-50" y="-50" width="100" height="100" fill="hsl(280,60%,70%)" rx="5"/>'
    ...
    def _dalle(self, title, cat):
        try:
            ...
            r = c.images.generate(
                model="dall-e-3",
                prompt=f"crochet {cat}: {title}. {self.config.style} illustration.",
    ...
    def _gemini(self, title, cat):
        try:
            ...
            r = m.generate_content(
                f"Generate image: crochet {cat}: {title}. {self.config.style}.",
                generation_config={"response_mime_type": "image/png"},
            )
            if r.parts and r.parts[0].inline_data:
                return base64.b64encode(r.parts[0].inline_data.data).decode()
            return self._placeholder(title, cat)
        except:
            return self._placeholder(title, cat)
    ...
    def _sd(self, title, cat):
        try:
            ...
            r = replicate.run(
                ...
                input={"prompt": f"crochet {cat}: {title}. {self.config.style}."},
            )
...
def generate_pattern_image(
    title, category="", provider="placeholder", api_key=None, style="watercolor"
):
    return ImageProvider(
        ImageConfig(provider=provider, api_key=api_key, style=style)
