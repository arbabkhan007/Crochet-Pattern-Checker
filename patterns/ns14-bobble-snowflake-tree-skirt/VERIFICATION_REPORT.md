...
Confirmed by two independent implementations that share no code
(`verify.py` and `crosscheck.py`) plus a line-by-line read of the document.
...
python3 crosscheck.py original.md   # exit 1  (second opinion)
python3 crosscheck.py corrected.md  # exit 0
...
## Second opinion (`crosscheck.py`)

Because a verifier written alongside a pattern can quietly agree with its own
assumptions, the result was reproduced by a second program built on a different
method: bracket repeats are expanded *textually* before anything is interpreted,
costs come from a separate matcher, and the column test asks a different question —
*is each increase worked into the stitch the previous round's increase produced,
and does each bobble sit on the stitch just after one?*

| | original | corrected |
|---|---|---|
| increases off-column | **360 of 360** | **0 of 360** |
| bobbles off-column | 12 on every bobble round | 0 |
| rounds with a slipped column | 30 of 30 | 0 of 30 |

Both programs agree, by different routes, on both files.

One honest caveat, recorded because it nearly produced a false clean bill of
health: the cross-checker's *first* column test traced each stitch's ancestry back
to round 1 and required the 12 increases to descend from the same 12 round-1
stitches. That test **passed the broken pattern**. A one-stitch slip still lands
inside the same ancestor's widening wedge, so the root set never changes. The test
is rotation-invariant and therefore blind to exactly this defect. It was replaced
by the column-continuity test above and is retained only as a weak sanity check.
...
### Found by reading, not by simulation

Three things no checker could have caught, found on a line-by-line pass:

- **The ch-24 claim was wrong.** The first draft of the fix said ch 24 gives a
  1.5-2.1 in opening *at rest*. It does not: R1 still has only 12 dc, and those
  same 12 stitches gather either ring to the same 4 in span. A longer chain raises
  the **stretched maximum** (to about 2.4 in) but barely moves the relaxed size.
  Reworded to say so.
- **R33 was referenced but never written.** The troubleshooting entry offered an
  extra plain round after Large without giving it. R33 is now spelled out in full
  and generated by the same code that builds the tables, so it cannot drift.
- **"Scallop count becomes 2 x N" was too vague** where a maker needs a number.
  Now: R15 -> 180 sts / 30 scallops, R24 -> 288 / 48, R33 -> 396 / 66.
...
| `crosscheck.py` | independent second opinion, shares no code with `verify.py` |
...
| `run-crosscheck-*.log` | second-opinion output for both files |

Regression tests live in `tests/test_ns14_tree_skirt.py` (6 tests, stdlib only).
Two of them assert that the **original** still fails, so the detectors cannot rot
into passing everything — which is precisely how the repository's bundled
validator ended up reporting `1 stitch` per round and `PASS` for all input.
