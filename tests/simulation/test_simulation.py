...
from crochet_checker.simulation.mesh import (
    Mesh,
    Vec3,
...
from crochet_checker.simulation.surface import (
    DetectedShape,
...
class TestMesh:
    ...
    def test_obj(self):
        ...
        m.add_face(0, 1, 2)
        obj = m.to_obj()
        assert "v 0" in obj and "f 1 2 3" in obj


class TestMeshGen:
    def test_sphere(self):
        m = generate_sphere_mesh(5.0, 16, 8)
        assert m.vertex_count > 0 and m.face_count > 0

    def test_tube(self):
        m = generate_tube_mesh(3, 3, 10, 16)
        assert m.vertex_count > 0

    def test_circle(self):
        m = generate_flat_circle_mesh(5.0, 24)
        assert m.face_count == 24

    def test_hat(self):
        m = generate_hat_mesh(5, 7, 8, segments=24)
        assert m.vertex_count > 0
...
class TestSurface:
    def test_generates_mesh(self):
        ...
        m = simulate_surface(parse_pattern(t))
        assert m.vertex_count > 0

    def test_obj_export(self):
        t = chr(10).join(
            ["Round 1: 6 sc into magic ring (6)", "Round 2: (sc, inc) x 6 (18)"]
        )
        m = simulate_surface(parse_pattern(t))
        obj = m.to_obj()
