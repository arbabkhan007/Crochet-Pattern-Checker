...
from crochet_checker.ai import (
    explain_pattern,
...
class TestSuggestions:
    ...
    def test_with_errors(self):
        ...
        if r.errors:
            assert len(generate_suggestions(p, r)) > 0
...
class TestDescription:
    ...
    def test_skill(self):
        assert (
            "beginner" in generate_description(parse_pattern(P)).skill_level.lower()
            or "easy" in generate_description(parse_pattern(P)).skill_level.lower()
        )
