...
from crochet_checker.ai_providers import AIConfig, AIProvider
...
class TestAI:
    def test_rule_explain(self):
        ...
        assert (
            len(AIProvider(AIConfig(provider="rule_based")).explain_pattern(p, r)) > 0
        )
    ...
    def test_translate(self):
        assert (
            "dc"
            in AIProvider(AIConfig()).translate_terminology("6 sc", "US", "UK").lower()
        )
    ...
    def test_fallback(self):
        ...
        assert (
            len(
                AIProvider(AIConfig(provider="openai", api_key=None)).explain_pattern(
                    p, r
                )
            )
            > 0
